...
export function AutocompleteSelect({ allLabel = 'All organisations', emptyLabel = 'No organisations found.', label, onChange, options, placeholder, value }: AutocompleteSelectProps) {
    ...
    <label className="relative block w-72 max-w-full">
      ...
      {isOpen ? (
        <div className="absolute left-0 right-0 top-[4.25rem] z-30 max-h-72 overflow-y-auto rounded-xl border border-gray-200 bg-white py-1 shadow-xl">
          ...
          {filteredOptions.length === 0 ? <p className="px-3 py-3 text-sm text-gray-400">{emptyLabel}</p> : null}
        </div>
      ) : null}
    </label>
  );
