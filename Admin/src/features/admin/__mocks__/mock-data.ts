import type { AdminData, AppFlagSummary, FeatureAudienceGroup, FeatureFlagDefinition, FeaturePlatform, KillSwitchEntry, Organisation, OrganisationMember, PreapprovedMember, Team, UserSummary } from '../types';
...
export const mockAdminData: AdminData = {
  ...
  domains: [
    { id: 'd1', name: 'app.acme.com', label: 'Acme App', secretAge: '12 days', secretOld: false, users: 487, orgs: 8, status: 'active', twoFaPolicy: 'optional', allowedEmailDomains: [], allowedEmails: [], allowedRedirectUrls: [], created: '2024-01-15', hash: 'a3f8d2c1...' },
    { id: 'd2', name: 'widgets.io', label: 'Widgets', secretAge: '89 days', secretOld: true, users: 312, orgs: 5, status: 'active', twoFaPolicy: 'optional', allowedEmailDomains: [], allowedEmails: [], allowedRedirectUrls: [], created: '2023-11-02', hash: 'b7e2f9a4...' },
    { id: 'd3', name: 'portal.example.com', label: 'Example Portal', secretAge: '5 days', secretOld: false, users: 198, orgs: 3, status: 'active', twoFaPolicy: 'optional', allowedEmailDomains: [], allowedEmails: [], allowedRedirectUrls: [], created: '2024-03-01', hash: 'c1d4e8b3...' },
    { id: 'd4', name: 'startup.dev', label: 'Startup Dev', secretAge: '22 days', secretOld: false, users: 104, orgs: 2, status: 'active', twoFaPolicy: 'optional', allowedEmailDomains: [], allowedEmails: [], allowedRedirectUrls: [], created: '2024-02-10', hash: 'e4b7c9f1...' },
    { id: 'd5', name: 'old-product.com', label: 'Old Product', secretAge: null, secretOld: false, users: 89, orgs: 2, status: 'disabled', twoFaPolicy: 'optional', allowedEmailDomains: [], allowedEmails: [], allowedRedirectUrls: [], created: '2023-06-10', hash: 'd9c3a7e2...' },
  ],
  organisations: [
  ...
  handshakeErrors: [
    handshakeError(
      ...
      'Config JWT signature did not match the configured JWKS key.',
      ['The JWT was signed with a key that does not match widgets.io.', 'Check that the JWT kid exists in the configured JWKS and that the domain claim matches config_url.'],
      [],
      { alg: 'RS256', typ: 'JWT', kid: 'widgets-prod' },
      {
        domain: 'widgets.io',
        app_identifier: 'io.widgets.dashboard',
        redirect_urls: ['https://widgets.io/auth/callback'],
    ...
    handshakeError(
      ...
      'startup',
      400,
      'CONFIG_MISSING_REQUIRED_CLAIM',
      'Config JWT was accepted but required claims were missing.',
      ['The config payload does not include enabled_auth_methods.', 'The OAuth popup cannot decide which sign-in methods to render.'],
      ['enabled_auth_methods', 'redirect_urls'],
      { alg: 'RS256', typ: 'JWT', kid: 'acme-staging' },
      {
        domain: 'app.acme.com',
        app_identifier: 'com.acme.main',
        enabled_auth_methods: '[missing]',
        redirect_urls: '[missing]',
        ui_theme: { primary: '#4f46e5' },
        shared_secret: '[redacted]',
      },
      ['shared_secret', 'refresh_token', 'client_secret'],
    ),
    handshakeError(
      'he3',
      '2026-04-21 11:46:27',
      ...
      ['Request host was startup.dev.', 'Config JWT domain claim was staging.startup.dev.', 'The app must issue a config JWT for the exact domain being authenticated.'],
      [],
      { alg: 'RS256', typ: 'JWT', kid: 'startup-dev' },
      {
...
function handshakeError(
  ...
  organisation: string,
  endpoint: string,
  phase: AdminData['handshakeErrors'][number]['phase'],
  statusCode: number,
  errorCode: string,
  summary: string,
  details: string[],
  ...
  return {
    ...
    missingClaims,
    ip: '185.220.101.xx',
    userAgent: 'uoa-sdk/2.0 node-fetch/3.3',
    requestId: `req_${id}_20260421`,
    requestJson: { config_url: `https://${domain}/auth/config`, source: 'query.config_url' },
    responseJson: { status: statusCode, content_type: 'text/plain' },
    jwtHeader,
    jwtPayload,
    redactions,
  };
...
function org(
  ...
  return { id, name, slug, allowedEmailDomains: [], allowedEmails: [], created, owner: { id: owner.id, name: owner.name, email: owner.email }, twoFaPolicy: 'inherit', teams, members, preapprovedMembers };
}

function team(id: string, orgId: string, name: string, description: string, isDefault: boolean, members: number): Team {
  return { id, orgId, name, description, isDefault, members, allowedEmailDomains: [], allowedEmails: [] };
}
