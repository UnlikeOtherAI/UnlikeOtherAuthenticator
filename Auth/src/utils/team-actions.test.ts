...
describe('submitTeamSelection', () => {
  ...
  it('keeps the legacy empty team selection wrapper available', async () => {
    const spy = vi.spyOn(api, 'selectTeam').mockResolvedValue({
      ok: true,
      status: 200,
      data: { ok: true, redirect_to: 'https://client.example.com/cb' },
...
describe('submitOrganisationCreation', () => {
  it('creates a team through the dedicated SSO endpoint', async () => {
    const spy = vi.spyOn(api, 'createOrganisation').mockResolvedValue({
      ok: true,
      status: 200,
      data: { ok: true, redirect_to: 'https://client.example.com/cb' },
    });
