 ...
 * `LoginForm`, and every team-chooser card so the branching logic only lives once.
 */
export type TeamResponseOutcome =
  | { kind: 'chooser'; loginToken: string; choices: SessionChoices }
...
export function interpretTeamResponse(data: unknown): TeamResponseOutcome {
 ...
 * the chooser-producing endpoints, since the caller already holds one) — into `SessionChoices`.
 * Returns null for anything that doesn't look like a valid payload (generic failure upstream).
 */
export function toSessionChoices(data: unknown): SessionChoices | null {
 ...
 * user can add a team or organisation, keep the chooser visible so that action remains
 * reachable.
 */
export function pickAutoSkipTeam(choices: SessionChoices): TeamChoice | null {
  if (
 ...
 */
export function pickHintTeam(choices: SessionChoices, teamHint: string | null): TeamChoice | null {
  const hint = teamHint?.trim();
  if (!hint) return null;
  return choices.teams.find((team) => team.teamId === hint || team.slug === hint) ?? null;
}

/** The subset of `usePopup()` needed to act on a `TeamResponseOutcome`. */
export type TeamOutcomeActions = {
 ...
 * and every team-chooser card hand off to once they have a response. Returns false for
 * `{ kind: 'error' }` so the caller can show its own generic error copy.
 */
export function applyTeamOutcome(
  outcome: TeamResponseOutcome,
  actions: TeamOutcomeActions,
): boolean {
  switch (outcome.kind) {
    case 'chooser':
      actions.setLoginToken(outcome.loginToken);
      actions.setSessionChoices(outcome.choices);
      actions.setView('team-chooser');
      return true;
    case 'redirect':
    ...
    case 'expired':
      ...
      actions.setSessionChoices(null);
