...
describe('parsePopupQueryParams', () => {
  ...
  it('parses the inviter-given name, trimmed, and ignores a blank one', () => {
    ...
    expect(parsePopupQueryParams('?invite_token=t').inviteName).toBeNull();
  });
  ...
  it('parses a login_token seeded alongside flow=team_chooser (social-callback bridge)', () => {
    const parsed = parsePopupQueryParams(
      '?config_url=https%3A%2F%2Fclient.example.com%2Fauth%2Fconfig&login_token=bridge.jwt&flow=team_chooser',
    );

    expect(parsed.loginToken).toBe('bridge.jwt');
  });

  it('ignores a login_token without the flow=team_chooser marker', () => {
