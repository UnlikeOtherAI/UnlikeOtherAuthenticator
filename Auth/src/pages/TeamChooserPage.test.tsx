...
import { TeamChooserPage } from './TeamChooserPage.js';
...
type ChooserFixture = Omit<SessionChoices, 'creatable_orgs'> &
  Partial<Pick<SessionChoices, 'creatable_orgs'>>;
...
function renderChooser(
  ...
  const choices: SessionChoices | null = fixture ? { creatable_orgs: [], ...fixture } : null;
...
describe('TeamChooserPage SSR rendering', () => {
  ...
  it('renders the title and the email-scoped subtitle', () => {
    ...
    expect(html).toContain('Your teams for jo@example.com');
  });
  ...
  it('renders an inline first-team form when there is no team destination yet', () => {
    ...
    const withExistingTeams = renderChooser({
      teams: [
        { teamId: 't1', orgId: 'o1', name: 'Backend Team', role: 'member' },
  ...
  // “Org and team should be different — org is a level above”: the page lists team groups,
  ...
  it('renders one opaque, card-corner create control for server-marked organisations', () => {
    ...
    // The org headings remain <h2>s so screen readers can jump between team groups.
    expect(html).toMatch(/<h2[^>]*>Acme<\/h2>/);
    expect(html).toMatch(/<h2[^>]*>Globex<\/h2>/);
    expect(html).toContain('aria-label="Create team"');
    // It overlaps the outer chooser card, never the first team row.
  ...
  it('offers creation for an org the user has no team in yet', () => {
    const html = renderChooser({
      teams: [
        { teamId: 't1', orgId: 'o1', name: 'General', role: 'admin', orgName: 'Acme' },
    ...
    expect(html).toContain('aria-label="Create team"');
    // Initech is a valid popup destination, not an empty team-list section.
    expect(html).not.toMatch(/<h2[^>]*>Initech<\/h2>/);
  });

  it('renders the team avatar image rather than the initials badge', () => {
    const html = renderChooser({
      teams: [
        {
          teamId: 't1',
