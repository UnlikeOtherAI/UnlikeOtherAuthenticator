...
import { CreateFirstTeamForm } from '../components/team/CreateFirstTeamForm.js';
import { CreateTeamDialog } from '../components/team/CreateTeamDialog.js';
import { InviteCard } from '../components/team/InviteCard.js';
import { TeamList } from '../components/team/TeamList.js';
import { usePopup } from '../hooks/use-popup.js';
import { useTheme } from '../hooks/use-theme.js';
import { useTranslation } from '../i18n/use-translation.js';
import type { AuthFlowQuery } from '../utils/api.js';
import { submitSessionChoices, submitTeamSelection } from '../utils/team-actions.js';
import {
  applyTeamOutcome,
  pickAutoSkipTeam,
  pickHintTeam,
...
/**
 * Phase 3c (design §11.2): the Slack "choose a team" screen. Reads `loginToken` +
 * `teamChoices` from `usePopup()` — set by `CodeEntryPage` or a chooser-producing
 ...
 * but no `teamChoices` yet, it hydrates them itself via `POST /auth/session-choices`.
 */
export function TeamChooserPage(): React.JSX.Element {
  ...
  // teamChoices yet is the social-callback hydration path below, not an invalid state.
  useEffect(() => {
    if (!loginToken) setView('login');
  }, [loginToken, setView]);

  // Phase 3c follow-up (design §4.3 Task 7 remainder): hydrate the chooser payload when we landed
  // here via the social-callback redirect (loginToken seeded, teamChoices not — CodeEntryPage
  // and the chooser-producing LoginForm always set both together, so this is a no-op for them).
  useEffect(() => {
    if (!loginToken || teamChoices || hydrateStarted.current) return;
  ...
  }, [loginToken, teamChoices, query, setSessionChoices, handleOutcome, t]);
  ...
  useEffect(() => {
    if (!loginToken || !teamChoices || autoSkipStarted.current) return;
    const skipTeam = pickAutoSkipTeam(teamChoices) ?? pickHintTeam(teamChoices, teamHint);
    if (!skipTeam) return;
  ...
  }, [loginToken, teamChoices, teamHint, query, handleOutcome]);

  if (!loginToken || !teamChoices) {
    if (hydrateFailed) {
      return (
        <div>
  ...
  if (skipTeam && !autoSkipFailed) {
    return (
      <div>
        <p className="text-sm text-[var(--uoa-color-muted)]">{t('teamChooser.autoSkip')}</p>
      </div>
    );
  }

  const hasInvites = teamChoices.pending_invites.length > 0;
  const hasTeams = teamChoices.teams.length > 0;
  // Orgs this user may add a team to (server-decided: ACTIVE owner/admin + the domain's
  // `allow_user_create_team`). Unlike `can_create_org` below this is NOT limited to users with no
  // team yet — creating a team inside an org you already run is an ordinary action.
  const creatableOrgs = teamChoices.creatable_orgs;
  ...
  const canCreateNewOrganisation = teamChoices.can_create_org;
  // With no usable team or invitation there is no organisation destination to pick. Put the
  // first-team form directly in the chooser rather than stranding it behind a floating modal.
  const showInlineFirstTeamForm =
    canCreateNewOrganisation && !hasTeams && !hasInvites && creatableOrgs.length === 0;
  // The inline first-team form already exposes the only available creation destination, so
  // do not duplicate it with the floating trigger. Every other permitted destination opens the
  // custom picker.
  const canOpenCreateDialog =
    creatableOrgs.length > 0 || (canCreateNewOrganisation && !showInlineFirstTeamForm);
    ...
    <div className="flex flex-col gap-4">
      {canOpenCreateDialog ? (
        <button
          ...
          aria-label={t('team.createDialog.open')}
          title={t('team.createDialog.open')}
      ...
      <div>
        <h1 className={`text-balance ${classNames.title}`}>{t('auth.teamChooser.title')}</h1>
        {pendingEmail ? (
          <p className="mt-1 text-sm text-[var(--uoa-color-muted)]">
            {t('teamChooser.subtitle', { email: pendingEmail })}
          </p>
        ) : null}
      </div>
      ...
      {showInlineFirstTeamForm ? (
        <CreateFirstTeamForm loginToken={loginToken} query={query} onOutcome={handleOutcome} />
      ) : null}

      {hasInvites ? (
        <div className="flex flex-col gap-3">
          {teamChoices.pending_invites.map((invite) => (
            <InviteCard
              key={invite.inviteId}
              invite={invite}
