...
- Confidential `/auth/token` assertions must be RS256, short-lived, audience-bound, and verified through the source config's same-host JWKS. Always re-resolve the current user and source-domain role; when an assertion includes workspace context, re-resolve its current org/team membership before issue.
