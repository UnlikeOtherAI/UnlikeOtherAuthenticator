...
- **[`Docs/Auth/user-settings.md`](./Docs/Auth/user-settings.md)** — Optional namespaced per-user settings (`/settings/me`). Defines the namespace → key → JSON value model, quotas, write semantics, and the cross-product trust model.
...
- User settings (`/settings/me`) are visible to every product the user signs into and are therefore never a place for secrets. The acting user is always the access-token subject; values are size-, depth- and quota-capped, and a PATCH is all-or-nothing including the quota check. See `Docs/Auth/user-settings.md`.
