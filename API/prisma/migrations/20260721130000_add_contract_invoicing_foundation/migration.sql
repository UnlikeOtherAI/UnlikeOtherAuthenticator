...
CREATE TYPE "BillingInvoicePaymentEventSource" AS ENUM ('MANUAL');
CREATE TYPE "BillingInvoiceAddonCollection" AS ENUM ('STRIPE_SEPARATE');
...
CREATE TABLE "billing_invoice_credit_settlement_references" (
  "id" TEXT NOT NULL,
  "invoice_id" TEXT NOT NULL,
  "service_id" TEXT NOT NULL,
  "settlement_id" TEXT NOT NULL,
  "adjustment_id" TEXT NOT NULL,
  "credits_applied_microcredits" BIGINT NOT NULL,
  "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT "billing_invoice_credit_settlement_references_pkey" PRIMARY KEY ("id"),
  CONSTRAINT "billing_invoice_credit_refs_amount_check" CHECK (
    "credits_applied_microcredits" >= 0
    AND "credits_applied_microcredits" % 10 = 0
  )
);
CREATE TABLE "billing_invoice_addon_lines" (
  "id" TEXT NOT NULL,
  "invoice_id" TEXT NOT NULL,
  "service_id" TEXT NOT NULL,
  "service_identifier" VARCHAR(100) NOT NULL,
  "service_name" VARCHAR(120) NOT NULL,
  "addon_subscription_id" TEXT NOT NULL,
  ...
  "collection" "BillingInvoiceAddonCollection" NOT NULL DEFAULT 'STRIPE_SEPARATE',
  "position" INTEGER NOT NULL,
  "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT "billing_invoice_addon_lines_pkey" PRIMARY KEY ("id"),
  CONSTRAINT "billing_invoice_addon_lines_offer_version_check" CHECK ("offer_version" > 0),
  CONSTRAINT "billing_invoice_addon_lines_amount_check" CHECK ("monthly_amount_minor" >= 0),
  CONSTRAINT "billing_invoice_addon_lines_currency_check" CHECK ("currency" ~ '^[A-Z]{3}$'),
  CONSTRAINT "billing_invoice_addon_lines_position_check" CHECK ("position" >= 0)
);
...
CREATE UNIQUE INDEX "billing_invoice_credit_refs_invoice_settlement_key"
  ON "billing_invoice_credit_settlement_references"("invoice_id", "settlement_id");
CREATE UNIQUE INDEX "billing_invoice_credit_refs_invoice_adjustment_key"
  ON "billing_invoice_credit_settlement_references"("invoice_id", "adjustment_id");
CREATE INDEX "billing_invoice_credit_refs_service_idx"
  ON "billing_invoice_credit_settlement_references"("service_id");
CREATE INDEX "billing_invoice_credit_refs_settlement_idx"
  ON "billing_invoice_credit_settlement_references"("settlement_id");
CREATE INDEX "billing_invoice_credit_refs_adjustment_idx"
  ON "billing_invoice_credit_settlement_references"("adjustment_id");
CREATE UNIQUE INDEX "billing_invoice_addon_lines_invoice_subscription_key"
  ON "billing_invoice_addon_lines"("invoice_id", "addon_subscription_id");
CREATE UNIQUE INDEX "billing_invoice_addon_lines_invoice_position_key"
  ON "billing_invoice_addon_lines"("invoice_id", "position");
CREATE INDEX "billing_invoice_addon_lines_service_idx" ON "billing_invoice_addon_lines"("service_id");
CREATE INDEX "billing_invoice_addon_lines_offer_idx" ON "billing_invoice_addon_lines"("offer_id");
CREATE INDEX "billing_invoice_addon_lines_catalog_idx" ON "billing_invoice_addon_lines"("catalog_id");
CREATE INDEX "billing_invoice_addon_lines_subscription_idx"
  ON "billing_invoice_addon_lines"("addon_subscription_id");
...
ALTER TABLE "billing_invoice_metering_references" ADD CONSTRAINT "billing_invoice_metering_references_service_id_fkey"
  FOREIGN KEY ("service_id") REFERENCES "billing_services"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_credit_settlement_references" ADD CONSTRAINT "billing_invoice_credit_refs_invoice_fkey"
  FOREIGN KEY ("invoice_id") REFERENCES "billing_invoices"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_credit_settlement_references" ADD CONSTRAINT "billing_invoice_credit_refs_service_fkey"
  FOREIGN KEY ("service_id") REFERENCES "billing_services"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_credit_settlement_references" ADD CONSTRAINT "billing_invoice_credit_refs_settlement_fkey"
  FOREIGN KEY ("settlement_id") REFERENCES "billing_credit_usage_settlements"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_credit_settlement_references" ADD CONSTRAINT "billing_invoice_credit_refs_adjustment_fkey"
  FOREIGN KEY ("adjustment_id") REFERENCES "billing_credit_usage_settlement_adjustments"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_addon_lines" ADD CONSTRAINT "billing_invoice_addon_lines_invoice_fkey"
  FOREIGN KEY ("invoice_id") REFERENCES "billing_invoices"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_addon_lines" ADD CONSTRAINT "billing_invoice_addon_lines_service_fkey"
  FOREIGN KEY ("service_id") REFERENCES "billing_services"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_addon_lines" ADD CONSTRAINT "billing_invoice_addon_lines_subscription_fkey"
  FOREIGN KEY ("addon_subscription_id") REFERENCES "billing_recurring_addon_subscriptions"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_addon_lines" ADD CONSTRAINT "billing_invoice_addon_lines_offer_fkey"
  FOREIGN KEY ("offer_id") REFERENCES "billing_recurring_addon_offers"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE "billing_invoice_addon_lines" ADD CONSTRAINT "billing_invoice_addon_lines_catalog_fkey"
  FOREIGN KEY ("catalog_id") REFERENCES "billing_recurring_addon_catalogs"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
DECLARE activation_service_id TEXT;
BEGIN
  ...
  END IF;
  IF OLD."status" IN ('ACTIVE', 'TERMINATED')
    AND NEW."activated_at" IS DISTINCT FROM OLD."activated_at"
  THEN RAISE EXCEPTION 'contract activation timestamp is immutable'; END IF;
  IF OLD."status" = 'TERMINATED'
    AND NEW."terminated_at" IS DISTINCT FROM OLD."terminated_at"
  THEN RAISE EXCEPTION 'contract termination timestamp is immutable'; END IF;
  ...
  IF OLD."status" = 'DRAFT' AND NEW."status" = 'ACTIVE' THEN
    ...
    FOR activation_service_id IN
      SELECT term."service_id"
      FROM "billing_contract_service_terms" term
      WHERE term."contract_version_id" = (
        SELECT version."id"
        FROM "billing_organisation_contract_versions" version
        WHERE version."contract_id" = NEW."id"
          AND EXISTS (
            SELECT 1 FROM "billing_contract_service_terms" candidate_term
            WHERE candidate_term."contract_version_id" = version."id"
          )
        ORDER BY version."effective_from_month" DESC, version."version" DESC
        LIMIT 1
      )
      ORDER BY term."service_id"
    LOOP
      PERFORM uoa_lock_stripe_contract_scope(activation_service_id, NEW."org_id");
      IF uoa_stripe_scope_blocks_manual_contract(activation_service_id, NEW."org_id") THEN
...
CREATE OR REPLACE FUNCTION uoa_lock_stripe_contract_scope(candidate_service_id TEXT, candidate_org_id TEXT)
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    hashtextextended('uoa-stripe-org:' || candidate_service_id || ':' || candidate_org_id, 0)
  );
END;
$$;
CREATE OR REPLACE FUNCTION uoa_stripe_scope_blocks_manual_contract(candidate_service_id TEXT, candidate_org_id TEXT)
RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
  SELECT EXISTS (
    SELECT 1
    FROM "billing_stripe_checkout_sessions" checkout
    WHERE checkout."service_id" = candidate_service_id
      AND checkout."org_id" = candidate_org_id
      AND (
        checkout."status" IN ('creating', 'open')
        OR (
          checkout."status" = 'complete'
          AND (
            NOT EXISTS (
              SELECT 1 FROM "billing_stripe_subscriptions" subscription
              WHERE subscription."checkout_id" = checkout."id"
            )
            OR EXISTS (
              SELECT 1 FROM "billing_stripe_subscriptions" subscription
              WHERE subscription."checkout_id" = checkout."id"
    ...
    SELECT 1
    FROM "billing_stripe_subscriptions" subscription
    WHERE subscription."service_id" = candidate_service_id
      AND subscription."org_id" = candidate_org_id
      AND subscription."status" NOT IN ('canceled', 'incomplete_expired')
  );
$$;
...
RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
  SELECT EXISTS (
    ...
    JOIN "billing_contract_service_terms" term
      ON term."contract_version_id" = current_version."id"
     AND term."service_id" = candidate_service_id
    WHERE contract."org_id" = candidate_org_id
...
CREATE OR REPLACE FUNCTION uoa_guard_stripe_contract_exclusivity()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE is_blocking BOOLEAN;
BEGIN
  is_blocking := CASE
  ...
  END;
  IF NOT is_blocking THEN RETURN NEW; END IF;
  PERFORM uoa_lock_stripe_contract_scope(NEW."service_id", NEW."org_id");
  IF uoa_active_contract_covers_service(NEW."service_id", NEW."org_id") THEN
...
END;
$$;
CREATE TRIGGER billing_stripe_checkout_contract_exclusive
BEFORE INSERT OR UPDATE ON "billing_stripe_checkout_sessions"
FOR EACH ROW EXECUTE FUNCTION uoa_guard_stripe_contract_exclusivity();
CREATE TRIGGER billing_stripe_subscription_contract_exclusive
...
DECLARE contract_status "BillingOrganisationContractStatus"; version_currency CHAR(3); version_month CHAR(7); latest_revision INTEGER;
  invoice_credit_microcredits NUMERIC; expected_credit_minor NUMERIC; collector_settlement_id TEXT;
BEGIN
  ...
  IF NEW."status" = 'ISSUING' AND (TG_OP = 'INSERT' OR OLD."status" <> 'ISSUING') THEN
    ...
    SELECT COALESCE(sum("credits_applied_microcredits"), 0)
      INTO invoice_credit_microcredits
...
FOR EACH ROW EXECUTE FUNCTION uoa_guard_billing_invoice_child();
CREATE OR REPLACE FUNCTION uoa_guard_billing_invoice_credit_reference()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE invoice_row "billing_invoices"%ROWTYPE; settlement_row "billing_credit_usage_settlements"%ROWTYPE;
  adjustment_row "billing_credit_usage_settlement_adjustments"%ROWTYPE;
  credit_row "billing_credit_accounts"%ROWTYPE; term_tariff_id TEXT; latest_adjustment_id TEXT;
  invoice_created_in_transaction BOOLEAN;
BEGIN
  IF TG_OP <> 'INSERT' THEN RAISE EXCEPTION 'invoice credit settlement evidence is immutable'; END IF;
  SELECT * INTO invoice_row
  FROM "billing_invoices" WHERE "id" = NEW."invoice_id" FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'invoice credit settlement evidence is immutable';
  END IF;
  SELECT invoice.xmin = pg_current_xact_id()::xid
    INTO invoice_created_in_transaction
  FROM "billing_invoices" invoice WHERE "id" = NEW."invoice_id";
  IF invoice_row."status" <> 'DRAFT' OR NOT invoice_created_in_transaction
  THEN RAISE EXCEPTION 'invoice credit settlement evidence is immutable'; END IF;
  SELECT "tariff_id" INTO term_tariff_id FROM "billing_contract_service_terms"
  WHERE "contract_version_id" = invoice_row."contract_version_id"
  ...
  SELECT "id" INTO latest_adjustment_id
  FROM "billing_credit_usage_settlement_adjustments"
  WHERE "settlement_id" = settlement_row."id"
  ORDER BY "sequence" DESC LIMIT 1;
  IF term_tariff_id IS NULL
    OR settlement_row."status" <> 'APPLIED'
    OR settlement_row."service_id" IS DISTINCT FROM NEW."service_id"
    OR settlement_row."tariff_id" IS DISTINCT FROM term_tariff_id
    OR settlement_row."billing_month" IS DISTINCT FROM invoice_row."billing_month"
    OR settlement_row."currency" IS DISTINCT FROM 'USD'
    OR settlement_row."subscription_id" IS NOT NULL
    OR credit_row."org_id" IS DISTINCT FROM invoice_row."org_id"
    OR credit_row."currency" IS DISTINCT FROM 'USD'
    OR adjustment_row."settlement_id" IS DISTINCT FROM settlement_row."id"
    OR adjustment_row."service_id" IS DISTINCT FROM NEW."service_id"
    OR adjustment_row."id" IS DISTINCT FROM latest_adjustment_id
    OR adjustment_row."cumulative_credits_consumed_microcredits"
      IS DISTINCT FROM NEW."credits_applied_microcredits"
  THEN RAISE EXCEPTION 'invoice credit settlement evidence is incoherent' USING ERRCODE = '23503'; END IF;
  IF EXISTS (
    SELECT 1 FROM "billing_credit_invoice_lines"
    WHERE "settlement_id" = settlement_row."id" AND "status" IN ('CREATING', 'APPLIED')
  ) THEN RAISE EXCEPTION 'credit settlement already has a Stripe collector' USING ERRCODE = '23514'; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER billing_invoice_credit_settlement_refs_immutable
BEFORE INSERT OR UPDATE OR DELETE ON "billing_invoice_credit_settlement_references"
FOR EACH ROW EXECUTE FUNCTION uoa_guard_billing_invoice_credit_reference();
CREATE OR REPLACE FUNCTION uoa_guard_billing_invoice_addon_line()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE invoice_row "billing_invoices"%ROWTYPE; subscription_row "billing_recurring_addon_subscriptions"%ROWTYPE;
  offer_row "billing_recurring_addon_offers"%ROWTYPE; catalog_row "billing_recurring_addon_catalogs"%ROWTYPE;
  service_identifier VARCHAR(100); service_name VARCHAR(120);
  period_start TIMESTAMP; period_end TIMESTAMP; invoice_created_in_transaction BOOLEAN;
BEGIN
  IF TG_OP <> 'INSERT' THEN RAISE EXCEPTION 'invoice add-on lines are immutable'; END IF;
  SELECT * INTO invoice_row
  FROM "billing_invoices" WHERE "id" = NEW."invoice_id" FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'invoice add-on lines are immutable';
  END IF;
  SELECT invoice.xmin = pg_current_xact_id()::xid
    INTO invoice_created_in_transaction
  FROM "billing_invoices" invoice WHERE "id" = NEW."invoice_id";
  IF invoice_row."status" <> 'DRAFT' OR NOT invoice_created_in_transaction
  THEN RAISE EXCEPTION 'invoice add-on lines are immutable'; END IF;
  period_start := to_date(invoice_row."billing_month" || '-01', 'YYYY-MM-DD');
  period_end := period_start + interval '1 month';
  SELECT * INTO subscription_row FROM "billing_recurring_addon_subscriptions"
  WHERE "id" = NEW."addon_subscription_id";
  SELECT * INTO offer_row FROM "billing_recurring_addon_offers" WHERE "id" = NEW."offer_id";
  SELECT * INTO catalog_row FROM "billing_recurring_addon_catalogs"
  WHERE "id" = subscription_row."catalog_id";
  SELECT "identifier", "name" INTO service_identifier, service_name
  FROM "billing_services" WHERE "id" = NEW."service_id";
  IF NOT EXISTS (SELECT 1 FROM "billing_contract_service_terms"
      WHERE "contract_version_id" = invoice_row."contract_version_id"
        AND "service_id" = NEW."service_id")
    OR subscription_row."service_id" IS DISTINCT FROM NEW."service_id"
    OR subscription_row."org_id" IS DISTINCT FROM invoice_row."org_id"
    OR subscription_row."offer_id" IS DISTINCT FROM NEW."offer_id"
    OR subscription_row."catalog_id" IS DISTINCT FROM NEW."catalog_id"
    OR subscription_row."scope" IS DISTINCT FROM NEW."scope"
    OR subscription_row."initial_invoice_paid_at" IS NULL
    OR subscription_row."initial_invoice_id" IS NULL
    OR subscription_row."activation_webhook_event_id" IS NULL
    OR subscription_row."entitlement_activated_at" IS NULL
    OR subscription_row."entitlement_activated_at" >= period_end
    OR (subscription_row."entitlement_deactivated_at" IS NOT NULL
      AND subscription_row."entitlement_deactivated_at" <= period_start)
    OR subscription_row."offer_key" IS DISTINCT FROM NEW."offer_key"
    OR offer_row."service_id" IS DISTINCT FROM NEW."service_id"
    OR offer_row."version" IS DISTINCT FROM NEW."offer_version"
    OR offer_row."key" IS DISTINCT FROM NEW."offer_key"
    OR offer_row."name" IS DISTINCT FROM NEW."offer_name"
    OR offer_row."monthly_amount_minor" IS DISTINCT FROM NEW."monthly_amount_minor"
    OR offer_row."currency" IS DISTINCT FROM NEW."currency"
    OR catalog_row."offer_id" IS DISTINCT FROM offer_row."id"
    OR catalog_row."id" IS DISTINCT FROM NEW."catalog_id"
    OR catalog_row."monthly_amount_minor" IS DISTINCT FROM NEW."monthly_amount_minor"
    OR catalog_row."currency" IS DISTINCT FROM NEW."currency"
    OR NEW."service_identifier" IS DISTINCT FROM service_identifier
    OR NEW."service_name" IS DISTINCT FROM service_name
    OR NEW."collection" IS DISTINCT FROM 'STRIPE_SEPARATE'
  THEN RAISE EXCEPTION 'invoice add-on line is not an exact separately billed subscription'
    USING ERRCODE = '23503'; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER billing_invoice_addon_lines_immutable
BEFORE INSERT OR UPDATE OR DELETE ON "billing_invoice_addon_lines"
FOR EACH ROW EXECUTE FUNCTION uoa_guard_billing_invoice_addon_line();
CREATE OR REPLACE FUNCTION uoa_guard_credit_invoice_line_collector()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW."status" NOT IN ('CREATING', 'APPLIED') THEN RETURN NEW; END IF;
  PERFORM pg_advisory_xact_lock(
    hashtextextended('uoa-credit-collector:' || NEW."settlement_id", 0)
  );
  IF EXISTS (
    SELECT 1
    FROM "billing_invoice_credit_settlement_references" reference
    JOIN "billing_invoices" invoice ON invoice."id" = reference."invoice_id"
    WHERE reference."settlement_id" = NEW."settlement_id"
      AND invoice."status" IN ('ISSUING', 'ISSUED')
  ) THEN RAISE EXCEPTION 'credit settlement already has a manual invoice collector'
    USING ERRCODE = '23514'; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER billing_credit_invoice_lines_single_collector
BEFORE INSERT OR UPDATE ON "billing_credit_invoice_lines"
FOR EACH ROW EXECUTE FUNCTION uoa_guard_credit_invoice_line_collector();
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    ...
    'billing_invoice_metering_references', 'billing_invoice_credit_settlement_references',
    'billing_invoice_addon_lines', 'billing_invoice_number_sequences',
