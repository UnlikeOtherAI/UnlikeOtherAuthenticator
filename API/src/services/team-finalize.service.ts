...
export type TeamFinalizeOutcome =
 ...
 * login.ts / social-callback branching, or (b) finalize and issue the (optionally team-scoped)
...
export async function finalizeWithTwoFaPolicy(
  ...
  if (!deps?.policyLockHeld) {
    ...
    return runInTransaction(transactionPrisma, async (tx) => {
      ...
      return finalizeWithTwoFaPolicy(params, {
        ...
        twoFaPolicyPrisma: deps?.twoFaPolicyPrisma ?? deps?.policyPrisma ?? tx,
  ...
  if ((params.orgId === undefined) !== (params.teamId === undefined)) {
    throw new Error('team scope requires both orgId and teamId');
  }
  const team =
    params.orgId === undefined ? {} : { orgId: params.orgId, teamId: params.teamId as string };

  // When team selection is transactional, use that transaction so a
  ...
  const finalResult = await finalizeAuthenticatedUser(
    ...
    deps?.prisma
      ? {
          ...
          // Keep code/signature-continuation issuance in the caller's team
          // selection transaction. A replay collision can then roll back every
          // grant or invite mutation produced by the losing request.
          signatureDeps: {
            prisma: deps.prisma,
            teamPrisma: deps.teamPrisma ?? deps.prisma,
          },
          teamPrisma: deps.teamPrisma,
        }
      : undefined,
  );
