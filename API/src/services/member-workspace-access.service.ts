...
export type MemberWorkspaceAccess = {
  ...
  /** Whether the selected organisation member is actively in this workspace. */
  hasAccess: boolean;
};

export type MemberWorkspaceAccessResult = {
  data: MemberWorkspaceAccess[];
  permissions: { changeWorkspaceAccess: boolean };
};

/**
 * A manager's editable portion of one member's workspace access.
 *
 ...
 */
export async function listMemberWorkspaceAccess(
  params: {
    orgId: string;
    domain: string;
    actorUserId?: string;
  ...
  deps?: OrgServiceDeps,
): Promise<MemberWorkspaceAccessResult> {
  ...
  if (scope.teamIds?.length === 0) {
    return { data: [], permissions: { changeWorkspaceAccess: false } };
  }
