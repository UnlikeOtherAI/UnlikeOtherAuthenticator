...
const PRODUCT_TEAM_POLICY_LOCK = 'uoa:product-team-policy:v1';

/** Concurrent auth readers hold this through their token transaction commit. */
export async function lockProductTeamPolicyShared(
  ...
  await prisma.$queryRaw(
    Prisma.sql`
      SELECT pg_advisory_xact_lock_shared(
        hashtextextended(${PRODUCT_TEAM_POLICY_LOCK}, 0)
      )::text AS "lockResult"
    `,
  );
}
...
export async function lockProductTeamPolicyExclusive(
  prisma: ProductPolicyLockPrisma,
): Promise<void> {
  if (typeof prisma.$queryRaw !== 'function' && !getEnv().DATABASE_URL) return;
  await prisma.$queryRaw(
