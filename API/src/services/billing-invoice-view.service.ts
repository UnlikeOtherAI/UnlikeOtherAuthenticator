...
type AddonLineValue = {
  ...
  collection: BillingInvoiceAddonCollection;
  position: number;
};
...
export function serializeCustomerSafeInvoice(invoice: CustomerSafeInvoice) {
  ...
  return {
    ...
    separately_billed_add_ons: [...invoice.addonLines]
      .sort((left, right) => left.position - right.position)
      .map((line) => ({
        id: line.id,
        service: {
        ...
        offer: { key: line.offerKey, name: line.offerName },
        scope: line.scope.toLowerCase(),
        collection: 'collected_separately',
        monthly_price: money(line.monthlyAmountMinor, line.currency),
