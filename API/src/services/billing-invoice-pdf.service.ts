...
const TEMPLATE_VERSION = 'uoa-contract-invoice-v3';
...
function drawAt(page: PDFPage, font: PDFFont, text: string, x: number, y: number, size = 10): void {
...
function drawSeparatelyBilledAddons(context: RenderContext, invoice: CustomerSafeInvoice): void {
  if (invoice.addonLines.length === 0) return;
  context.y -= 8;
  ensureSpace(context, 48);
  drawFlowLines(context, 'Recurring add-ons collected separately', {
    font: context.bold,
    size: 10,
  });
  drawFlowLines(context, 'These prices are not included in this invoice total.', { size: 8 });
  context.y -= 4;
  for (const line of [...invoice.addonLines].sort((a, b) => a.position - b.position)) {
    const label = `${line.offerName} — ${line.serviceName} (${line.scope.toLowerCase()})`;
    const labelLines = wrapText(label, context.font, 9, 350);
    const rowHeight = Math.max(18, labelLines.length * 13 + 5);
    ensureSpace(context, rowHeight);
    labelLines.forEach((value, index) => {
      drawAt(context.page, context.font, value, MARGIN, context.y - index * 13, 9);
    });
    const price = `${displayMinor(line.monthlyAmountMinor, line.currency)}/month`;
    drawAt(
      context.page,
      context.font,
      price,
...
function drawTotal(context: RenderContext, label: string, amount: string, bold = false): void {
...
export async function generateBillingInvoicePdf(invoice: CustomerSafeInvoice): Promise<Uint8Array> {
  ...
  drawSeparatelyBilledAddons(context, invoice);
