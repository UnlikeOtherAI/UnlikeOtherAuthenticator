...
function rejectWorkspaceCreation(): never {
 ...
 */
export function registerAuthCreateWorkspaceRoute(app: FastifyInstance): void {
  app.post(
    ...
    async (request, reply) => {
      ...
      const outcome = await runInTransaction(request.adminDb, async (tx) => {
        ...
        // those bridges cannot create two first workspaces concurrently.
        ...
        // and all workspace effects back, leaving a legitimate retry possible.
        ...
        if (!team) rejectWorkspaceCreation();
        ...
        if (!user) rejectWorkspaceCreation();
