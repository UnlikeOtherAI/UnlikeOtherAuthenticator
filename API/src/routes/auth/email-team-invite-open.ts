...
import { emailTeamInviteOpenRateLimiter } from './rate-limit-keys.js';

const ParamsSchema = z.object({
  inviteId: z.string().trim().min(1).max(256),
});
...
export function registerAuthEmailTeamInviteOpenRoute(app: FastifyInstance): void {
  app.get(
    '/auth/email/team-invite-open/:inviteId.gif',
    { preHandler: [emailTeamInviteOpenRateLimiter] },
    async (request, reply) => {
