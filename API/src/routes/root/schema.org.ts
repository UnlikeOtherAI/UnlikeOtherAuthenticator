...
export const orgEndpoints: EndpointSchema[] = [
  ...
  },
  {
    ...
    path: '/org/organisations',
    description:
      'Create organisation owned by the calling user (X-UOA-Access-Token). Non-superusers also require org_features.allow_user_create_org=true, else 403 ORG_CREATION_NOT_ALLOWED. In backend mode (no user credential) the body must carry owner_user_id instead and allow_user_create_org does not apply. Without slug, the address is derived from name; a derived or chosen address another organisation on the domain already holds is refused with 400 ORG_SLUG_TAKEN (never suffixed), so ask the person for another name. The response is the organisation record plus defaultTeam: the first team, named after the organisation and created in the same transaction, with the owner already an ACTIVE member of it. Use defaultTeam.id to address the new team immediately — no follow-up read is needed, and none is possible with a subject assertion, which must already name the org and team it acts on.',
  ...
  {
    method: 'GET',
    path: '/org/organisations/:orgId/members/:userId/teams',
    description:
      'List the selected member\'s editable team access. The result contains only teams where the caller currently holds members.manage; it never exposes or changes memberships outside that scope.',
    auth: 'domain hash bearer token',
    response: {
      data: 'array — { id, name, slug, avatarImageUrl, hasAccess }',
      permissions: 'object — { changeTeamAccess }; live caller verdict',
    },
  },
  {
    method: 'PUT',
