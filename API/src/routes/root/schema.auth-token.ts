...
export const authTokenEndpoint: EndpointSchema = {
  ...
  path: '/auth/token',
  description:
    'Exchange an authorization code or refresh token for the legacy access + refresh pair, explicitly move that pair to one exact authorized workspace, or exchange a source-signed JWT assertion / UOA-issued audience-bound access token for a resource-bound confidential access token. Authorization-code exchange re-resolves current exact-workspace 2FA policy and user enrollment and rejects a code lacking required interactive TOTP proof before token-family creation.',
  auth: 'config_url query param + domain hash bearer token',
  body: {
    'grant_type?':
      '"authorization_code" (default), "refresh_token", "urn:unlikeotherai:params:oauth:grant-type:workspace-switch", or "urn:ietf:params:oauth:grant-type:token-exchange"',
    ...
    'refresh_token?': 'refresh token (for refresh_token and workspace-switch grants)',
    'organization_id?':
      'exact target organisation id (required for workspace-switch grant; must be paired with team_id)',
    'team_id?':
      'exact target team id (required for workspace-switch grant; must belong to organization_id)',
    'subject_token?':
  ...
  response: {
    ...
    'refresh_token?':
      'string — opaque, server-side only; authorization-code/refresh/workspace-switch grants only, never hand to the browser',
    'refresh_token_expires_in?':
      'number — seconds until refresh_token expiry; authorization-code/refresh/workspace-switch grants only',
    'issued_token_type?':
    ...
    'workspace-switch errors':
      'Before an edge commits, 403 WORKSPACE_NOT_AVAILABLE means the exact pair is unavailable or policy denies it and 403 INTERACTION_REQUIRED means fresh interactive assurance is needed; both leave the source live. 409 WORKSPACE_SWITCH_CONFLICT means the deterministic family already advanced under another grant or target and does not revoke that valid family. If an exact response-loss retry finds its already-committed target no longer policy-valid, UOA retires only that family and returns authenticated 401 INVALID_REFRESH_TOKEN. Invalid/post-grace reuse also returns that stable code while keeping the underlying reason opaque; client-authentication failures remain generic.',
    'refresh response-loss recovery':
      "For 120 seconds after rotation, retrying the same predecessor with the same authenticated app credential, grant intent, exact workspace target, and client context returns the verified current successor. Every descendant must retain that operation's expected scope; a later scope transition produces WORKSPACE_SWITCH_CONFLICT instead of returning changed scope. Outside the window, any predecessor use revokes the family and prior access-token version.",
  },
};
