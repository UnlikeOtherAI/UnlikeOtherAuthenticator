...
- \`ui_theme\` is required. See \`/api\` for the full contract (colors as hex only, radii/font sizes as CSS lengths, button + card styles, logo with required \`url\` and \`alt\`). Image logos have gently rounded corners by default; set \`ui_theme.logo.rounded: false\` to preserve square corners. The rounding uses \`ui_theme.radii.button\` and does not affect text logos.
