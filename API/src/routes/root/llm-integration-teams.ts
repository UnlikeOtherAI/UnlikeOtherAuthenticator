...
Backend acceptance: \`POST /org/organisations/:orgId/teams/:teamId/invitations/:inviteId/accept\` with strict body \`{ "userId": "<UOA invitee id>" }\` accepts that exact invitation server-to-server. Authenticate with the domain-hash bearer and verified config, enable \`org_features.backend_org_management\`, and **omit** \`X-UOA-Access-Token\`; any present user-token header is \`401 ACCESS_TOKEN_NOT_ALLOWED\`. The backend asserts the invitee id within backend mode's existing trust boundary. Success is \`200 { "ok": true, "orgId": "…", "teamId": "…" }\`; replaying the same accepted invite for the same user is the same success. Unknown or path/domain-mismatched ids, email mismatch, revoked, expired, and approval-pending invitations all retain the generic \`400\` body. If the invitee already belongs to another organisation on this invitation's origin domain, the body additionally carries public code \`ORG_CONFLICT_ON_DOMAIN\`: retrying that acceptance can never succeed, and the caller already holds the exact invite id plus backend authority, so the code is not an enumeration oracle.
