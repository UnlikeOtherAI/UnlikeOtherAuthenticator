...
Team invites now carry an \`expiresAt\` (24 hours from send/resend — resending refreshes both the invite and emailed-token deadlines) and an \`approvalStatus\`: \`not_required\` | \`pending\` | \`approved\` | \`denied\`. The derived invite \`status\` gains \`expired\` and \`revoked\` alongside \`pending | accepted | declined | replaced\` (\`replaced\` = superseded by a newer invite for the same email; \`revoked\` = explicitly cancelled); an expired, revoked, or not-yet-approved invite cannot be accepted and is excluded from the team chooser / \`firstLogin.pending_invites\`. The mailbox-bound HTML journey says \`This invitation has expired\` only for an elapsed invitation/token deadline and \`This invitation can’t be used\` for every other unusable state.

The invitation e-mail is addressed to the invitee and names the inviter by display name only (never by address): its subject reads \`<Inviter> invited you to join <team> on <product>\`, or \`You’ve been invited to join …\` when UOA has no name for the inviter. The product is \`ui_theme.logo.alt\`; it is left out when the config has none. Filter on the sender address, not the subject — the subject is copy and may change.
...
Backend acceptance: \`POST /org/organisations/:orgId/teams/:teamId/invitations/:inviteId/accept\` with strict body \`{ "userId": "<UOA invitee id>" }\` accepts that exact invitation server-to-server. Authenticate with the domain-hash bearer and verified config, enable \`org_features.backend_org_management\`, and **omit** \`X-UOA-Access-Token\`; any present user-token header is \`401 ACCESS_TOKEN_NOT_ALLOWED\`. The backend asserts the invitee id within backend mode's existing trust boundary. Success is \`200 { "ok": true, "orgId": "…", "teamId": "…" }\`; replaying the same accepted invite for the same user is the same success. Unknown or path-mismatched ids, email mismatch, revoked, expired, and approval-pending invitations all retain the generic \`400\` body. The organisation's origin domain is never compared with the calling product's — every client domain is equal, and one organisation is usable from every UOA-integrated product, so an invitation into an organisation founded through another product is accepted normally. An invitee may hold active memberships in more than one organisation on the same origin domain.

Revoking an invitation: \`DELETE /org/organisations/:orgId/teams/:teamId/invitations/:inviteId\` cancels a pending invitation in either state — already emailed, or still awaiting member-invite approval. The row is kept (invite history is audit history) with \`status: "revoked"\`, outstanding emailed tokens are consumed, and the invite link's hosted landing page answers with the generic \`This invitation can’t be used\` page (it never says which unusable state it is). In user mode the caller must be an org/team \`owner\`/\`admin\` or the invite's original inviter; backend mode follows the table above. Responses: \`200 { "ok": true }\` (idempotent — an already-revoked or declined invitation also answers \`200\`), \`409\` with code \`INVITATION_ALREADY_ACCEPTED\` once the invite has been accepted (remove the member instead), generic \`404\` for an unknown id or a foreign org/team.
