...
2. **Icon URL** — the team's \`iconUrl\` (the externally hosted team icon from \`PUT /org/organisations/:orgId/teams/:teamId\`). UOA proxies it server-side under the same HTTPS-only, SSRF-guarded, ~5s/5 MiB rules and reports \`X-UOA-Avatar-Source: provider\`. Nothing is persisted, and \`iconUrl\` itself is never modified by the avatar endpoints.
...
**The public read.** \`GET /teams/:teamId/avatar\` takes no credential and no \`?domain=\`, so a browser can render a team logo straight from an \`<img src>\`. Its response explicitly carries \`Cross-Origin-Resource-Policy: cross-origin\`; credentialed avatar routes retain the default same-origin policy. It exists because the auth-window team chooser renders in a popup that holds no bearer of any class — every other form above needs one, which a plain \`<img>\` cannot send. It is not an existence oracle: an unknown or deleted team id returns the same deterministic generated SVG a real team with no image would, so every id answers 200 and no id can be probed. Team ids are unguessable cuids; anyone holding one can fetch that team's logo. Reads are rate-limited 300/hour per IP plus a global ceiling of 120,000/hour service-wide, and no \`config_url\` is accepted (an anonymous caller must not be able to aim the config fetcher), so the generated style is always the platform default.
...
Team records in JSON follow the same identity rule as users: **every team record carries \`avatarImageUrl\`**, an absolute URL that always resolves to an image and is fetchable with the credential class you already used — \`<PUBLIC_BASE_URL>/domain/teams/<teamId>/avatar?domain=<domain>\` on ordinary \`/org/*\` and \`/internal/org/*\` records, \`<PUBLIC_BASE_URL>/internal/admin/teams/<teamId>/avatar\` on \`/internal/admin/*\`. The cross-product \`GET /org/me\` team directory uses the public form described below so every returned team remains renderable regardless of its owning domain. It is never null. \`iconUrl\` is unchanged and keeps its own meaning: the external URL you set, or \`null\` if you set none.
