...
**The public read.** \`GET /teams/:teamId/avatar\` takes no credential and no \`?domain=\`, so a browser can render a workspace logo straight from an \`<img src>\`. Its response explicitly carries \`Cross-Origin-Resource-Policy: cross-origin\`; credentialed avatar routes retain the default same-origin policy. It exists because the auth-window workspace chooser renders in a popup that holds no bearer of any class — every other form above needs one, which a plain \`<img>\` cannot send. It is not an existence oracle: an unknown or deleted team id returns the same deterministic generated SVG a real team with no image would, so every id answers 200 and no id can be probed. Team ids are unguessable cuids; anyone holding one can fetch that workspace's logo. Reads are rate-limited 300/hour per IP plus a global ceiling of 120,000/hour service-wide, and no \`config_url\` is accepted (an anonymous caller must not be able to aim the config fetcher), so the generated style is always the platform default.
