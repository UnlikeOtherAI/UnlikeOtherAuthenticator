import {
  llmBackendModeErrorTableMarkdown,
  llmBackendModeRouteTableMarkdown,
  llmConfidentialDelegationMarkdown,
} from './llm-confidential-delegation.js';
...
${llmConfidentialDelegationMarkdown}### 4.6b Server-to-server organisation/team management (backend mode)
...
Team invites grant \`member\` or \`admin\` only — \`owner\` is never invitable (it can only come from direct membership management); any owner grant is rejected with the generic 400, and the same rail is a database CHECK constraint.
