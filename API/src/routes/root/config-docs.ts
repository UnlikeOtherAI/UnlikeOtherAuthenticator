...
export const configJwtDocumentation = {
  ...
  optional_fields: {
    ...
    org_features: {
      ...
      capabilities:
        "string[] (optional) — the consuming product's declared capability catalogue, mirrored here so role_grants can be validated against it. UOA's own names (members.manage, teams.manage) are always valid on top of this list",
      role_grants:
        'object (optional) — { "org": { <role>: <capability>[] }, "team": { <role>: <capability>[] } }. Which roles hold which capabilities; UOA gates its own team surfaces on capability names, never role strings. Rules: "owner" (org or team) implicitly holds every capability and MUST NOT appear here; a role the table does not mention holds nothing (no coercion to "member"); a workspace answer is the union of the org-role and team-role grants, so an org grant of a team-scope capability reaches into every team. Rejected at config load if a role is not in the matching vocabulary, a capability is undeclared, "owner" is named, or the scope key is unknown. Absent means the legacy default table: {"org":{"admin":["members.manage","teams.manage"]},"team":{"admin":["members.manage","teams.manage"]}}',
