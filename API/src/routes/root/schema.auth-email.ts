...
export const authEmailEndpoints: EndpointSchema[] = [
  ...
  },
  {
    ...
    description:
      'Email registration/login link landing. A direct password invitation without PKCE opens account creation with the invitation email fixed in a read-only field; the invitation token, not that field, remains authoritative. The account-creation screen also pre-fills its editable name field with the name the inviter gave (carried as invite_name, which is sent whenever the invitation has one). An invitation for an address that already has an account carries a LOGIN_LINK token and likewise arrives without PKCE: it is accepted here and answered with a terminal confirmation page, because there is no code verifier to end in an authorization code and a login restart would drop the invitation. A refused invitation acceptance answers 400 rather than showing a login form: an elapsed invite/token deadline renders "This invitation has expired", while revoked, used, conflicting, and every other unusable state render "This invitation can’t be used". For a non-invite link with config.login_flow.team_selection="auto", 2+ ACTIVE teams, a pending invite, or zero teams with can_create_org redirect to the team chooser; exactly one ACTIVE team/no invite is selected server-side and carried through applicable 2FA into the code. Invite-bound links carry the accepted invite orgId/teamId through the same 2FA/code/token pipeline without showing the chooser. team_selection="off" leaves legacy clients unscoped, but a recognized product pre-binds one exact team before 2FA without showing the chooser.',
    query: {
      state:
  ...
  },
  {
    ...
    description:
      'Team invite landing page with accept/decline actions. Emailed invitations are valid for 24 hours. The page names the inviter when UOA knows their name. An elapsed invitation or email-token deadline renders "This invitation has expired"; every other unusable state renders "This invitation can’t be used".',
    query: {
      token: 'string (required)',
      config_url: 'string (required)',
      redirect_url:
