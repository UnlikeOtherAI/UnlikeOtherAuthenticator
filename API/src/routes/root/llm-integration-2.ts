...
2. \`POST /auth/verify-code?config_url=...\` — body \`{ email, code }\`. IP + email rate-limited; wrong
   ...
   error. On success:
   - \`workspace_selection: "auto"\` → \`{ login_token, teams: [{ teamId, orgId, name, slug, role, iconUrl, avatarImageUrl }], pending_invites: [{ inviteId, teamName, invitedBy }], can_create_org }\`.
