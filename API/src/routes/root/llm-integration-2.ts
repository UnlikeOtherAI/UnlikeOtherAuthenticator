...
2. \`POST /auth/verify-code?config_url=...\` — body \`{ email, code }\`. IP + email rate-limited; wrong
   ...
   error. On success:
   - \`team_selection: "auto"\` → \`{ login_token, teams: [{ teamId, orgId, name, slug, role, iconUrl, avatarImageUrl, orgName, orgSlug }], pending_invites: [{ inviteId, teamName, orgName, invitedBy }], can_create_org, creatable_orgs }\`.
...
When it is one of your config's own \`redirect_urls\`, UOA renders a
"Continue to &lt;product&gt;" button pointing at it, naming the product from
\`ui_theme.logo.alt\` (falling back to the config domain), and the invitation email names the
product too. A value that is not in \`redirect_urls\` is dropped, not rejected: the pages read
exactly as they do without the parameter, and never redirect anywhere you did not allow-list.
...
- \`DELETE /internal/admin/organisations/:orgId\` — superuser-only organisation deletion. Nested membership rows cascade while UOA user rows remain; organisations with protected billing/commercial records return \`400\` with public code \`ORG_HAS_PROTECTED_RECORDS\`.
