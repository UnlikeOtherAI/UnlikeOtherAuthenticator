import { tryParseHttpUrl } from './http-url.js';
 ...
 * trailing-slash stripping, which would change what matches). Each entry must be a valid http(s)
 * URL, matching the config `redirect_urls` contract. An empty list means "no admin additions".
 */
