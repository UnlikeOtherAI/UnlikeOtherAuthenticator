...
const userEmail = 'single-team-social@example.com';
...
describe.skipIf(!hasDatabase)('social auto-selected team token flow', () => {
  ...
  it('binds the sole ACTIVE team to the code, access token, refresh row, and rotation', async () => {
    const domainAuth = await createAdminDomain(
      {
        domain,
        clientSecret: 'single-team-test-client-secret-0123456789',
    ...
    const organisation = await handle!.prisma.organisation.create({
      data: {
        domain,
        name: 'Single Team Org',
        slug: 'single-team-org',
        ownerId: user.id,
      },
