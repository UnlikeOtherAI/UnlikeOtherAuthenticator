...
function actionContext(creditAccount: BillingCreditAccount) {
  return {
  ...
  it('keeps a late SetupIntent from superseding a newer consent generation', async () => {
    ...
    await updateBillingCreditAutoTopUp(
      ...
      },
      {
        ...
        resolveContext: vi.fn().mockResolvedValue(actionContext(original)),
  ...
  it('rejects disable when manager authority is revoked after authorization', async () => {
    ...
    await expect(
      disableBillingCreditAutoTopUp(
        ...
        {
          prisma: handle!.prisma,
          resolveContext: vi.fn().mockResolvedValue(actionContext(preauthorized)),
        },
      ),
    ).rejects.toBeDefined();
