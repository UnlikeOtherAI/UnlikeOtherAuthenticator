...
describe.skipIf(!hasDatabase)('email exact-one team 2FA completion', () => {
  ...
  async function createUserTeam(params: {
    email: string;
    twoFaSecret?: string;
  ...
  it('verify-email challenge completion issues a code for the sole team', async () => {
    ...
    const team = await createUserTeam({ email, twoFaSecret: totpSecret });
    ...
    try {
      ...
      expect(challenge).toMatchObject({ orgId: team.orgId, teamId: team.teamId });
      ...
      const storedCode = await handle.prisma.authorizationCode.findFirstOrThrow({
        where: { userId: team.userId, usedAt: null },
        select: { orgId: true, teamId: true },
      });
      expect(storedCode).toEqual({ orgId: team.orgId, teamId: team.teamId });
    } finally {
      await app.close();
    }
  });

  it('email-link required enrollment completion issues a code for the sole team', async () => {
    await handle.prisma.clientDomain.update({
      where: { domain },
    ...
    const team = await createUserTeam({ email });
    await createEmailToken(email, rawToken);
    await installConfig();
    ...
    try {
      ...
      expect(setupClaims).toMatchObject({ orgId: team.orgId, teamId: team.teamId });
  ...
  it('verify-email returns the create-team chooser for a new zero-team user', async () => {
    const email = 'verify-create-team@example.com';
    const rawToken = 'verify-create-team-token';
    await createEmailToken(email, rawToken);
  ...
  it('email-link redirects a new zero-team user to the create-team chooser', async () => {
    const email = 'link-create-team@example.com';
    const rawToken = 'link-create-team-token';
    await createEmailToken(email, rawToken);
    await installConfig({ allowUserCreateOrg: true });
    ...
    try {
      ...
      expect(response.statusCode, response.body).toBe(302);
      const redirect = new URL(response.headers.location as string, 'http://localhost');
      expect(redirect.searchParams.get('flow')).toBe('team_chooser');
