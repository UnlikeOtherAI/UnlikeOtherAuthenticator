...
import { buildSessionChoices } from '../../src/services/first-login.service.js';
import { resolveProductTeamPolicy } from '../../src/services/product-team-policy.service.js';

const runsAgainstProductionRoles =
  process.env.RUN_PRODUCT_TEAM_RLS_TESTS === 'true' &&
 ...
 *
 * RUN_PRODUCT_TEAM_RLS_TESTS=true pnpm --filter @uoa/api exec vitest run \
 *   tests/integration/product-team-policy-rls.test.ts
 */
describe.skipIf(!runsAgainstProductionRoles)(
  'product team policy through production uoa_app/uoa_admin roles',
  () => {
    ...
    it('denies product control-plane reads to uoa_app and exposes choices only through uoa_admin', async () => {
      ...
      await expect(
        resolveProductTeamPolicy({ domain: fixture.domain }, { prisma: appPrisma }),
      ).rejects.toBeTruthy();
