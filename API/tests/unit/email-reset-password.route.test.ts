...
vi.mock('../../src/services/auth-reset-password.service.js', () => ({
  validatePasswordResetToken: (...args: unknown[]) => validatePasswordResetTokenMock(...args),
}));
...
describe('GET /auth/email/reset-password', () => {
  beforeEach(() => {
  ...
  it('renders the login screen instead of an auth error when a reset link is already used', async () => {
    validatePasswordResetTokenMock.mockRejectedValue(
  ...
  it('renders the login screen when a reset link has expired (the page-refresh case)', async () => {
    validatePasswordResetTokenMock.mockRejectedValue(
      new AppError('BAD_REQUEST', 400, 'TOKEN_EXPIRED'),
    );
    ...
    expect(renderAuthEntrypointHtmlMock).toHaveBeenCalledWith(
      expect.objectContaining({
        requestUrl: expect.not.stringContaining('email_token'),
      }),
    );
  ...
  it('does not swallow non-token failures (e.g. infrastructure errors)', async () => {
    validatePasswordResetTokenMock.mockRejectedValue(
      new AppError('INTERNAL', 500, 'DATABASE_DISABLED'),
    );
    ...
    // The login fallback must not be rendered for a real failure.
