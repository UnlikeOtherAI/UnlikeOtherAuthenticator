...
import {
  CLIENT_DOMAIN_TEAM_POLICY,
  resolveProductTeamPolicy,
} from '../../src/services/product-team-policy.service.js';

describe('product team policy', () => {
  it('binds one active client origin to one active customer-lifecycle product', async () => {
    ...
    await expect(
      resolveProductTeamPolicy({ domain: 'API.DeepSignal.Live.' }, { now: () => now, prisma }),
    ).resolves.toEqual({
  ...
  it.each(['inactive', 'pending', 'disabled'])(
    ...
    async (status) => {
      ...
      expect(policy).toEqual(CLIENT_DOMAIN_TEAM_POLICY);
