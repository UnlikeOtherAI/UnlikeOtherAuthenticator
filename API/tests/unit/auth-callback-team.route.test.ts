...
describe('GET /auth/callback/:provider team selection', () => {
  beforeEach(() => {
  ...
  it('keeps the chooser and PKCE context for multiple ACTIVE teams', async () => {
    ...
    expect(location.searchParams.get('flow')).toBe('team_chooser');
  ...
  it('carries the auto-selected team through social 2FA', async () => {
  ...
  it('carries the auto-selected team into required 2FA enrollment', async () => {
  ...
  it('preserves the chooser for a zero-team user who can create a team', async () => {
    buildSessionChoicesMock.mockResolvedValue({
      teams: [],
      pending_invites: [],
      can_create_org: true,
  ...
  it('leaves the code unscoped when team selection is off', async () => {
  ...
  it('pre-binds the recognized product team before required social enrollment', async () => {
    validateConfigFieldsMock.mockReturnValue(
      baseConfig({
        '2fa_enabled': true,
        login_flow: { email_code_enabled: false, team_selection: 'off' },
