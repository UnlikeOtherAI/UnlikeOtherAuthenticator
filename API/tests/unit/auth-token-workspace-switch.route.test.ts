...
const exchangeWorkspaceSwitchForTokensMock = vi.fn();
...
vi.mock('../../src/services/workspace-switch-token.service.js', async () => {
  const actual = await vi.importActual<
    typeof import('../../src/services/workspace-switch-token.service.js')
  >('../../src/services/workspace-switch-token.service.js');
  return {
    ...actual,
    exchangeWorkspaceSwitchForTokens: (...args: unknown[]) =>
      exchangeWorkspaceSwitchForTokensMock(...args),
  };
});
...
const grantType = 'urn:unlikeotherai:params:oauth:grant-type:workspace-switch';
...
describe('POST /auth/token workspace-switch grant', () => {
  ...
  beforeEach(() => {
    ...
    exchangeWorkspaceSwitchForTokensMock.mockResolvedValue({
  ...
  it('dispatches the exact target and returns the standard pair without firstLogin', async () => {
    ...
    try {
      ...
      expect(exchangeWorkspaceSwitchForTokensMock).toHaveBeenCalledWith(
        {
  ...
  it.each([
    ...
    ['wrong grant literal', { grant_type: 'workspace_switch' }],
    ...
    try {
      ...
      expect(exchangeWorkspaceSwitchForTokensMock).not.toHaveBeenCalled();
    } finally {
      await app.close();
    }
  });
  ...
  it.each([
    ...
    exchangeWorkspaceSwitchForTokensMock.mockRejectedValueOnce(new AppError(appCode, status, code));
    const app = await createTestApp();
    try {
      const response = await app.inject({
        method: 'POST',
  ...
  it.each([
    ['WORKSPACE_NOT_AVAILABLE', 403, 'FORBIDDEN'],
    ['INTERACTION_REQUIRED', 403, 'FORBIDDEN'],
    ['WORKSPACE_SWITCH_CONFLICT', 409, 'BAD_REQUEST'],
  ...
  it('exposes the stable invalid-refresh code after client authentication', async () => {
    exchangeWorkspaceSwitchForTokensMock.mockRejectedValueOnce(
