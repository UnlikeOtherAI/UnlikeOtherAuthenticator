...
function baseConfig(allowTeamCreation = true): ClientConfig {
  return {
    ...
    org_features: {
      ...
      allow_user_create_org: allowTeamCreation,
...
async function postCreateTeam(body: Record<string, unknown>) {
  const { registerAuthCreateOrganisationRoute } =
    await import('../../src/routes/auth/auth-create-organisation.js');
  const app = fastify();
  ...
  try {
    return await app.inject({
      method: 'POST',
      url: `/auth/create-organisation?${QUERY_SUFFIX}`,
      payload: body,
    });
  } finally {
    await app.close();
...
describe('POST /auth/create-organisation', () => {
  beforeEach(() => {
  ...
  it('rejects creation when the domain has not enabled self-service teams', async () => {
    currentConfig = baseConfig(false);
    const loginToken = await mintLoginToken();
    const res = await postCreateTeam({ login_token: loginToken, name: 'Acme Space' });
