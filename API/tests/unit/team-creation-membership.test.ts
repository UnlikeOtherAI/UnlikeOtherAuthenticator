...
import { resolveTeamCreatorTeamRole } from '../../src/services/role-grants.js';
import { makeConfig, makePrismaMock, now, useTeamServiceTestEnv } from './helpers/team-service-test-helpers.js';

/**
 * Creating a team has to leave the creator able to ENTER it.
 *
 ...
 * team creation from its own UI created a team its author could not open.
 */
...
describe('creating a team leaves its creator able to enter it', () => {
...
describe('the creator role respects the domain vocabulary', () => {
  it('is owner under the default vocabulary', () => {
    expect(resolveTeamCreatorTeamRole(makeConfig())).toBe('owner');
  });
  ...
  it('falls back to the most privileged configured role when owner is absent', () => {
    ...
    expect(resolveTeamCreatorTeamRole(config)).toBe('lead');
  });
});
