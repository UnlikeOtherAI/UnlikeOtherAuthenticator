...
describe('POST /auth/login — team chooser wiring (Phase 3b Task 7)', () => {
  beforeEach(() => {
  ...
  it('team_selection "auto" + 2FA satisfied: returns login_token + chooser instead of finalizing', async () => {
    currentConfig = baseConfig({
  ...
  it('resolves the exact auto-selected team before enforcing 2FA', async () => {
    currentConfig = baseConfig({
      '2fa_enabled': true,
      login_flow: { email_code_enabled: false, team_selection: 'auto' },
    });
    ...
    expect(signLoginSessionMock).not.toHaveBeenCalled();
    expect(buildSessionChoicesMock).toHaveBeenCalled();
  ...
  it('pre-binds a recognized product team before enrolled 2FA when the chooser is off', async () => {
    currentConfig = baseConfig({
      '2fa_enabled': true,
  ...
  it('pre-binds a recognized product team before required enrollment when chooser is off', async () => {
    currentConfig = baseConfig({
      '2fa_enabled': true,
      login_flow: { email_code_enabled: false, team_selection: 'off' },
    });
