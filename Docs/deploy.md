...
| `NESSIE_UOA_AUTOMATIC_MEMBERSHIP_CONTROL_URL` | Plain credential-free HTTPS Nessie base URL. UOA appends `/api/internal/uoa/automatic-membership/control`; configure it only together with the matching secret. The Admin UI remains fail-closed when absent. |
| `NESSIE_UOA_AUTOMATIC_MEMBERSHIP_CONTROL_SECRET` | Secret Manager: dedicated high-entropy UOA→Nessie bridge secret. Never expose it to Admin browsers or reuse a UOA domain/app key. Nessie verifies a 60-second HMAC request timestamp, request UUID, and signature over the exact JSON body. |
...
| `SHARED_SECRET`                             | Secret Manager: `uoa-auth-shared-secret`; generate with `openssl rand -base64 48`; a short or guessable value is brute-forceable offline from any leaked peppered digest, and rotating it later invalidates every domain-hash bearer and refresh token                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `USER_ACCESS_TOKEN_PRIVATE_JWK`             | Secret Manager: `uoa-auth-user-access-token-private-jwk`; dedicated current RS256 private RSA JWK for the user access token relying parties receive. Optional — unset leaves the token HS256 over `SHARED_SECRET`, which cannot be published and so cannot be verified by a relying party. Set it together with the public set; see `Docs/Auth/access-token-verification.md`                                                        |
| `USER_ACCESS_TOKEN_PUBLIC_JWKS_JSON`        | Secret Manager: `uoa-auth-user-access-token-public-jwks-json`; public-only JWKS with the current key plus retired keys still inside an unexpired token's TTL. Must contain the current private key's `kid`. Served at `GET /oauth/jwks.json`. UOA accepts HS256 and RS256 throughout, so enabling and rolling back are both safe                                                        |
...
| `BILLING_ACTOR_AUDIENCE_MODE`               | Plain `warn` (default) or `enforce`. Whether an `X-UOA-Actor` assertion naming only the legacy constant audience is accepted-and-logged or refused with `BILLING_ACTOR_AUDIENCE_MISMATCH`. An assertion naming the exact endpoint is accepted in both modes; anything else is refused in both. Flip to `enforce` only once the legacy-audience warning has been silent for every product — see `Docs/Auth/billing-actor-assertions.md`                                                     |
