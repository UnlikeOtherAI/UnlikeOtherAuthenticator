...
- Team-invite lifecycle decisions live in ONE place. `services/team-invite-state-machine.ts` is a pure module — no Prisma, no clock — owning the actionable/terminal/resolved predicates, the derived wire status, and the per-transition rules; `createTeamInvites`, `createMemberInvite`, `approveInvite`, `denyInvite`, `resendTeamInvite`, `revokeTeamInvite`, and both accept/decline paths all route through it, and `toInviteRecord` derives its `status` from it, so the status products read cannot drift from the rules that gate transitions. Transitions with an endpoint-specific outcome (revoke's 409 `INVITATION_ALREADY_ACCEPTED` and its idempotent success) are returned as a decision the caller maps, never thrown from the pure module. It deliberately holds no authorization: who may invite is a capability question owned by `role-grants.ts`, and which roles exist is per-domain config.
- Team-invite database invariants (`20260816140000_team_invite_actionable_invariants`): at most one actionable invite (unaccepted, undeclined, unrevoked, approval not DENIED) per `(team_id, lower(email))` via the partial unique index `team_invites_one_actionable_per_team_email`; `team_invites_team_role_check` refuses `owner` — deliberately `<> 'owner'` rather than an enumerated list, because the team-role vocabulary is per-domain config the database cannot see, with the per-domain half enforced by `normalizeInviteGrantRole`; and accepted/declined/revoked are mutually exclusive, `accepted_at` pairs with `accepted_user_id`, and an accepted invite is never PENDING/DENIED. Every CHECK is `NOT VALID`, so the deploy neither fails on nor rewrites historical rows, while Postgres still re-checks them on any future UPDATE. The migration first collapses duplicate actionable rows (earliest kept, rest revoked `REPLACED`) and terminalises actionable `owner` invitations (`REVOKED`), so the index build cannot fail on live data; accepted owner history is left untouched. A pre-existing row that already violated terminal coherence survives but is effectively frozen — any UPDATE of it now fails the re-check — which is harmless because every write path is scoped to actionable rows.
