...
Team-invite DB invariants (migrations `20260814130000_team_invite_delivery_foundation` + `20260814140000_team_invite_contract_alignment`): at most one actionable (unaccepted, undeclined, unrevoked, non-DENIED) invite per `(team, lower(email))`, enforced by the partial unique index `team_invites_one_actionable_per_team_email`; invite writes may grant `member`/`admin` only (`team_invites_team_role_check`, NOT VALID so accepted historical owner rows survive but are rechecked on update) — owner is never invitable, and the follow-up migration revoked every legacy actionable owner invite while preserving accepted owner history. The services enforce the same rail before any write via `normalizeInviteGrantRole` in `team-invite.service.base.ts` (backend bulk create, member-initiated create, and resend), so owner attempts fail with the generic BAD_REQUEST rather than a raw constraint violation. Accepted/declined/revoked are mutually exclusive and `accepted_at`/`accepted_user_id` must be paired. `team_invite_deliveries` is the non-secret durable email-delivery outbox (one row per invite + generation, leased PENDING/PROCESSING/SENT states, no token material); its `generation` and timestamps carry real database defaults (0 / CURRENT_TIMESTAMP) matching the Prisma `@default`s. It is admin/BYPASSRLS-only behind forced RLS like `confidential_delegation_mappings`. The pure decision module `services/team-invite-state-machine.ts` mirrors these invariants; the invite services are not yet rewired to the state machine, and no dispatcher/scheduler exists yet.
