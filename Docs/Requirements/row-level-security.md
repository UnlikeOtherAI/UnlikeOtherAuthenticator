...
13. **Pre-auth flows are _admin-side_ even though they pass `config-verifier`.** These routes have `app.domain` but no `app.user_id`:
    ...
    2. **Tenant context with `userId` set late** — use when the route _does_ produce a `userId` mid-handler (e.g. `/auth/login` after `loginWithEmailPassword`). Keep the pre-auth user lookup on `request.adminDb`; switch to `request.withTenantTx` for post-authentication writes when no capability must be consumed atomically across scopes.

    Concretely: fully _post-auth_ routes (access-token bearer required) normally go through
    `request.withTenantTx`. Narrow destructive membership endpoints are an explicit exception:
    org deactivation/removal and team-member removal must update refresh rows issued by sibling
    product domains, so they use `request.adminDb`, repeat exact actor/org/team authorization, and
    commit the status tombstone plus scoped revocation in one transaction. The `/auth/*` family is
    predominantly `request.adminDb`.

14. **`login_logs` are written post-auth with a known userId + domain**, so they belong in `withTenantTx`. Authorization-code exchange is the exception: the opaque code is the capability, so validation, ordered ACTIVE-membership row locks, one-time code consumption, refresh-token creation, and access-token context reads all run in one `request.adminDb` (BYPASSRLS) transaction. This prevents membership deactivation/removal from committing between the ACTIVE check and token creation. Refresh-token exchange and revoke also stay on `request.adminDb`.
...
- [ ] Per-route migration to `request.withTenantTx` — see section 16 for status per route family. Post-auth `/org/*` and authenticated `/twofactor/*` are the migration target except the documented cross-product membership-revocation boundaries; most `/auth/*` stays on `request.adminDb` per sharp-edge #13.
...
| `/auth/login`                   | T (same-domain post-auth writes) + A (pre-auth and product-workspace reads) | `loginWithEmailPassword` stays on `adminDb` for the email → user resolution. Same-domain authorization-code and login-log writes remain inside `withTenantTx`. A recognized product chooser resolves its control-plane policy and cross-domain ACTIVE memberships only through `adminDb`; the tenant transaction cannot read `client_domains`/`billing_app_keys` and cannot enumerate another org. |
...
| `/auth/revoke`                  | T                                                                           | Opaque domain-scoped lookup, then canonical user-global/user-domain locks and a post-lock re-read; family revocation and token-version bump share the tenant transaction.                                                                                                                                                                                                                          |
| `/auth/token-exchange`          | T (post-auth) + A (pre-auth lookup)                                         | Same split as `/auth/login`.                                                                                                                                                                                                                                                                                                                                                                       |
| `/auth/callback` (social)       | T (same-domain post-auth writes) + A (pre-auth and product-workspace reads) | Same split as `/auth/login`; social auto-selection receives the explicit admin policy/membership client.                                                                                                                                                                                                                                                                                           |

### `/org/*` (T by default; destructive cross-product revocation uses A)
...
| `/org/organisations/:orgId/members/*`                  | T / A    | Org member DELETE and deactivate use A for atomic cross-domain exact-org refresh revocation; other member routes use T. |
| `/org/organisations/:orgId/teams/*`                    | T        |                                                                                                                         |
| `/org/organisations/:orgId/teams/:teamId/members/*`    | T / A    | Team-member DELETE uses A for atomic cross-domain exact-team refresh revocation; other member routes use T.             |
...
| `/2fa/disable`      | A        | Exact access-token subject; global refresh revocation requires sibling-domain visibility. |
...
- [x] `/org/*` migrated to T except org member DELETE/deactivate and team-member DELETE, whose lifecycle transactions use A to revoke exact scoped refresh families across product domains.
...
| Confidential `/auth/token` mapping resolution, assertion identity/workspace reads + one-time `jti` claim                                                 | `uoa_admin`                                                                                           | Outbound/JWT verification occurs before the transaction. Final ClientDomain, delegation, server-owned product policy, product-aware exact org/team, replay claim, and signing share the global policy read lock and admin transaction. Recognized products cannot omit `active`.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Product workspace policy, pre-2FA exact placement, chooser expansion, post-2FA/signature scope validation, `firstLogin`, and scoped refresh revalidation | `uoa_admin`                                                                                           | `client_domains`, `billing_app_keys`, `billing_services`, and a selected cross-domain Organisation policy are inaccessible through `uoa_app`. Recognized products resolve exact scope before 2FA even when the chooser is off; code exchange rechecks the current policy/proof after locked membership validation. Issuance holds a shared global product-policy advisory lock; supported policy mutations hold its exclusive form. A per-user advisory lock serializes first placement. Email-link/verify-email choice resolution, placement, exact policy finalization, and immediate code issuance share one admin transaction so that lock survives through commit. Same-domain rows may remain legacy-unscoped. |
| Legacy `/auth/token` and browser-safe `/internal/admin/token` authorization-code exchange                                                                | `uoa_admin`                                                                                           | The code is the capability. Code validation/consumption, ordered ACTIVE org/team membership locks, refresh-token creation, and access-token context reads share one transaction, linearized with every membership-status writer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Legacy `/auth/token` refresh exchange                                                                                                                    | `uoa_admin`                                                                                           | The opaque refresh token is the capability. After lookup, exchange takes user-global then user+domain locks and re-reads; scoped rotation then locks exact organisation/team memberships. Reuse revocation commits before generic rejection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Explicit `/auth/token` workspace-switch grant                                                                                                            | `uoa_admin`                                                                                           | The opaque refresh token plus exact target is the capability. Under the shared product-policy fence and refresh session locks, UOA locks/revalidates source then target ACTIVE memberships, target 2FA assurance, and signature policy before writing the deterministic target-scoped successor. Ordinary replay stays source-scoped; switch replay stays exact-target-scoped. Valid semantic conflicts roll back without revocation, while post-grace predecessor use commits theft revocation before opaque rejection.                                                                                                                                                                                             |
| Password reset/binding and 2FA reset/disable                                                                                                             | `uoa_admin`                                                                                           | Exact-user credential mutation, all-domain refresh revocation, and token-version increment share one transaction under the user-global lock. Authenticated `/2fa/disable` uses the access-token subject, not tenant SQL, as its authorization boundary.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Org member deactivate/remove and team-member remove                                                                                                      | `uoa_admin`                                                                                           | The service repeats exact manager/org/team authorization, takes user-global before domain/membership locks, and commits status plus exact user+org or user+team refresh revocation across all issuing domains. The tenant role cannot see sibling-domain rows.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Interactive-login authorization-code issuance org lookup                                                                                                 | `uoa_app` for same-domain; explicit `uoa_admin` workspace client for a recognized cross-product scope | Same-domain uses the bootstrap predicate. A cross-product scope is never inferred from the tenant role and is revalidated at issuance, exchange, and refresh.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- **v0.5 — 2026-04-22** — Completed the deferred migrations from v0.4. Refactored `access-request-flow.service.ts::finalizeAuthenticatedUser` to accept `deps.prisma` and thread it through `handlePostAuthenticationAccessRequest` + `issueAuthorizationCode`. Wrapped `/twofactor/verify` in T (post-challenge flow, userId from challenge JWT). Wrapped full post-auth flow in `/auth/login`, `/auth/callback`, `/auth/token-exchange`, and `/internal/org/*` (groups / group-members / team-group-assignment) in T with domain-only tenant context; `setTenantContextFromRequest` assigns `{ orgId: null, userId: null }` so user lookups resolve under `app.domain` only. Side discoveries: (1) `tenant-context.plugin.ts` was using `decorateRequest(name, { getter })` which installs a getter-only prototype property that cannot be reassigned in onRequest under strict mode; replaced with plain-value decorators. (2) Plugin was not wrapped in `fastify-plugin`, so `request.withTenantTx` never propagated to the parent scope — dependency `fastify-plugin@5.1.0` added and plugin now exports `fp(plugin)`. (3) Plugin's `withTenantTx` gained a no-DB fallback: when `DATABASE_URL` is absent (unit-test scenarios that mock the service layer), it invokes the handler with a stand-in tx so route code stays uniform. Full API test suite green: 419 passed / 65 skipped / 0 failed.
- **v0.6 — 2026-07-19** — Legacy public and browser-safe Admin authorization-code exchange moved wholly onto one BYPASSRLS transaction. The transaction locks ACTIVE organisation/team memberships in the shared lifecycle order before consuming the code and creating the refresh/access pair, closing the deactivation race.
- **v0.7 — 2026-07-21** — Product domains bound by one active/current `CUSTOMER_LIFECYCLE` app-key service mapping may select any exact ACTIVE user org/team without circular direct-access evidence. Every control-plane and cross-org read is explicitly routed through `uoa_admin`; chooser, selection, 2FA/signature completion, authorization-code exchange, `firstLogin`, and refresh use the same policy. Unknown/revoked/ambiguous products retain same-domain behavior, pending invites remain same-domain, and an invalid scoped refresh fails without rotation or tenant switching. Added an opt-in production-role RLS canary.
- **v0.8 — 2026-07-21** — Org deactivation/removal and team-member removal became narrow `uoa_admin` lifecycle boundaries so status writes and exact scoped refresh-family revocation can commit together across issuing product domains. Refresh rotation takes the same organisation-then-team membership locks, closing replacement-after-revoke races. Legacy same-domain org-session revocation remains supported.
- **v0.9 — 2026-07-22** — Added exact user+domain transaction serialization and a post-lock row re-read to legacy refresh, org deactivation, and org removal, preventing an unscoped replacement from escaping same-domain revocation. Reuse-triggered family revocation now commits through a private transaction outcome before the ordinary invalid-refresh response is raised; current-token rotation shares the same lock.
- **v0.10 — 2026-07-22** — Added a canonical per-user global lock ahead of user/domain and workspace locks. Family logout now re-reads under that hierarchy and commits family revocation with `tokenVersion`; password reset/binding and every 2FA reset/disable path commit credential mutation, all-domain refresh revocation, and `tokenVersion` together through `uoa_admin`.
- **v0.11 — 2026-07-22** — Recognized-product scope is pre-bound before 2FA on every interactive identity path even when the chooser is off. Exact cross-domain Organisation policy participates strongest-wins, and authorization-code exchange transactionally rechecks current policy/enrollment against persisted TOTP proof before token-family creation.
- **v0.12 — 2026-08-13** — Added the explicit exact-target workspace-switch grant on the existing BYPASSRLS refresh transaction. Refresh families retain immutable authorization-code 2FA assurance, ordinary refresh cannot change scope, strict deterministic replay never returns a later differently scoped descendant, and current Organisation 2FA-policy writes join the exclusive product-policy fence.
