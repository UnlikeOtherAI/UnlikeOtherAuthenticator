...
13. **Pre-auth flows are _admin-side_ even though they pass `config-verifier`.** These routes have `app.domain` but no `app.user_id`:
    ...
    Concretely: only fully _post-auth_ routes (access-token bearer required) go cleanly through `request.withTenantTx`. This is the `/org/*`, `/twofactor/verify`, and `/internal/org/*` surface. The `/auth/*` family is predominantly `request.adminDb`.
...
| Legacy `/auth/token` refresh exchange                                                                                           | `uoa_admin`                                                                                           | The opaque refresh token is the capability and must be rotated atomically before caller tenancy exists.                                                                                                                                                                                                                                                                         |
...
- **v0.8 — 2026-07-21** — Recognized-product scope is pre-bound before 2FA on every interactive identity path even when the chooser is off. Exact cross-domain Organisation policy participates strongest-wins, and authorization-code exchange transactionally rechecks current policy/enrollment against persisted TOTP proof before token-family creation.
