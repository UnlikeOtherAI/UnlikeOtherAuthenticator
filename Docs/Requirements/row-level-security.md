...
| Legacy `/auth/token` refresh exchange                                                                                           | `uoa_admin`                                                                                           | The opaque refresh token is the capability. After lookup, exchange takes a narrow user+domain transaction lock and re-reads the row; scoped rotation then locks exact organisation/team memberships. Reuse revocation commits before the generic rejection is raised. |
| Org member deactivate/remove and team-member remove                                                                             | `uoa_admin`                                                                                           | The service repeats exact manager/org/team authorization, locks membership rows in canonical order, and commits status plus exact user+org or user+team refresh revocation across all issuing domains. The tenant role cannot see sibling-domain refresh rows. |
