...
13. **Pre-auth flows are *admin-side* even though they pass `config-verifier`.** These routes have `app.domain` but no `app.user_id`:
    ...
    2. **Tenant context with `userId` set late** — use when the route *does* produce a `userId` mid-handler (e.g. `/auth/login` after `loginWithEmailPassword`). Keep the pre-auth user lookup on `request.adminDb`; switch to `request.withTenantTx` for post-authentication writes when no capability must be consumed atomically across scopes.
...
| Interactive-login authorization-code issuance org lookup | `uoa_app` | Same bootstrap predicate; this is distinct from `/auth/token` capability exchange above. |
