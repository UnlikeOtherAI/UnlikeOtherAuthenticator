...
| Legacy `/auth/token` refresh exchange                                                                                           | `uoa_admin`                                                                                           | The opaque refresh token is the capability and must be rotated atomically before caller tenancy exists. Scoped rotation locks the exact organisation then team memberships before replacement, using the same order as lifecycle writers. |
