...
| Product workspace policy, chooser expansion, post-2FA/signature scope validation, `firstLogin`, and scoped refresh revalidation | `uoa_admin`                                                                                           | `client_domains`, `billing_app_keys`, and `billing_services` are admin-only; cross-org membership enumeration is impossible and forbidden through `uoa_app`. Issuance holds a shared global product-policy advisory lock; supported policy mutations hold its exclusive form. A per-user advisory lock serializes first placement. Same-domain rows may remain legacy-unscoped. |
