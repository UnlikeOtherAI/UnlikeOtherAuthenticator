...
Plus an "Invite people" action (the §4.7 user-facing invite endpoint) and the team's `joinPolicy` selector in workspace settings. Member list endpoints accordingly accept `?status=` and `?guests=` filters and default to `ACTIVE` (a behavioural addendum to §6's "Changed" table).
...
1. `POST /auth/select-team` accepts `{ login_token, inviteId, action: "accept" | "decline" }` — decline (Slack's invite-card Decline) marks the invite declined via the existing `declineTeamInviteByToken` logic, authenticated by the bridge token instead of the emailed token, and returns the refreshed chooser payload.
2. `GET /auth` gains optional `team_hint` query param (chooser preselect / skip; ignored when invalid).
3. `GET /org/me` enrichment: `orgName`, `iconUrl`, `status` filtering, `lastLoginAt` ordering, `pending_invites[]`.
4. Member list endpoints: `?status=`, `?guests=`, `include=invited` filters (formalizing §6).
5. Schema (added to migration step 5 in §7): `icon_url` on `teams` and `organisations`.
6. `/Auth` additions: `CodeEntryPage`, `WorkspaceChooserPage`, `/components/workspace/*`, `/ui/CodeInput.tsx` — `Docs/Auth/architecture-auth.md`'s directory tree and flow list must be updated in the implementation PR that adds them.

Phasing: items 1–2 and the new pages land with **Phase 3** (they are the chooser); item 3–4 with **Phase 2** (lifecycle read surface); item 5 with **Phase 1** (trivial additive columns).
