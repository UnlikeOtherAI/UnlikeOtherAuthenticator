...
- Write paths (`acceptTeamInviteWithinTransaction`, `org-placement`, `user-team-requirement`, auto-enrolment upserts) must not resurrect `DEACTIVATED` rows (`update: {}` upserts already have the right shape; add an explicit status guard so auto-enrolment does not silently re-activate a suspended member).
  - **Correction (2026-09-14):** for `acceptTeamInviteWithinTransaction` this guard covers `DEACTIVATED` only, as written above. A fresh valid invitation is a re-add in the sense of the previous bullet, so it flips a `REMOVED` org row and the invited team's `REMOVED` row back to `ACTIVE` (org role reset to `member`, team role from the invite, counted against limits, audited). A `DEACTIVATED` row is refused with `MEMBERSHIP_DEACTIVATED`. The 2026-07-19 hardening in §12a item 5 had widened the refusal to `REMOVED`, which made a removed person permanently un-invitable.
...
- Auth-code, refresh, workspace-switch, and confidential issuance hold a shared global product-policy advisory lock through commit; supported domain/service/lifecycle-key and Organisation 2FA-policy mutators hold its exclusive form. The authenticated ClientDomain is re-read under that lock. Confidential verification happens before the transaction, while final delegation, product-aware exact workspace validation, one-time assertion consumption, and signing happen inside it. Recognized product assertions must include `active`.
- **Workspace switching** ("add/switch workspaces from the sidebar"): shipped as an explicit backend-only custom grant on `POST /auth/token`, `urn:unlikeotherai:params:oauth:grant-type:workspace-switch`, requiring exact `{ refresh_token, organization_id, team_id }` and no operation id. It locks/revalidates source and target ACTIVE scopes, target 2FA assurance, product and signature policy before one deterministic target-scoped rotation. Ordinary refresh remains scope-stable. Exact in-window replay may recover only while every descendant retains the requested target; another grant/target or later switch is a non-revoking `WORKSPACE_SWITCH_CONFLICT`, while post-grace reuse still revokes. `WORKSPACE_NOT_AVAILABLE` and `INTERACTION_REQUIRED` are safe non-consuming 403 outcomes. The existing `team_hint` interactive path remains the fallback when stronger assurance is required.
...
```prisma
expiresAt      DateTime?             @map("expires_at")        // invite-level expiry (24 hours), matched by the email-token TTL
...
- **Expiry**: derived-status logic in `team-invite.service.base.ts` adds `expired`; the durable invite and its emailed token both last 24 hours. Expired invites can be re-sent (refreshing both deadlines), matching Slack's re-invite UX. `firstLogin.pending_invites` and the login chooser exclude expired/denied invites.
...
- **Invitation revocation** (added 2026-08-15, consumer-driven — Nessie): `DELETE /org/organisations/:orgId/teams/:teamId/invitations/:inviteId` cancels a pending invite in either state (emailed, or `approvalStatus: PENDING`). Storage stays honest instead of deleting rows: `TeamInvite.revokedReason` (`REPLACED | REVOKED`) records _why_ `revokedAt` was set — every pre-existing revoked row was implicitly revoked by replacement (the create/resend "supersede older unresolved invites" writers), and the migration backfills those as `REPLACED`; explicit revocation writes `REVOKED` and the derived status gains `revoked` beside `replaced`. Permission mirrors the member-invite matrix: org/team owner/admin or the invite's original inviter (still-ACTIVE, per the Phase 2 actor rule) in user mode; backend mode via the standard `requireOrgRole()` dual-mode guard (`backend_org_management` opt-in, `uoa_actor` audit provenance). Idempotent `200 { ok: true }` on an already-revoked/declined invite, `409 INVITATION_ALREADY_ACCEPTED` after acceptance, generic 404 otherwise. Revocation also consumes outstanding emailed tokens. The hosted journey distinguishes a genuine elapsed deadline as **Invitation expired**; revocation and every other unusable state render only **Invitation invalid**. A revoked invite can no longer be approved/denied through the approval queue.

- **Invitation read by id** (added 2026-08-15, consumer-driven — DeepSignal partner federation, useful to Nessie): `GET /org/organisations/:orgId/teams/:teamId/invitations/:inviteId` returns one invitation in exactly the `toInviteRecord` shape each entry of the list carries. It is the by-id companion to the list — a consumer that already holds an id (from a bulk-invite result, the list, or a resend) can re-read that one row instead of paging a team's whole invite history to find it, which is what a federated partner tracking a specific invitation actually needs. Authorization is the list's, not the revoke's: domain-hash + verified config + `org_features`, backend-mode capable, no `requireOrgRole()` — a read that returns what the list already returns must not be harder to reach than the list. Read-only: nothing is written and no audit row is produced (`invite.revoked` stays the only invite-lifecycle audit action). Unknown id, an invitation belonging to a foreign org/team, and a cross-domain org all answer the same generic `404`, so the endpoint is never an existence oracle. The id it consumes is the one the bulk-invite call already returns: `results[i].invite.id`, present on `invited` and `resent_existing` rows (the three no-invitation outcomes create no row and carry `email` + `status` alone). The member-initiated variant deliberately still answers `{ status: "ok" }` with no id — returning one would re-open the enumeration hole that response shape exists to close.
...
| `POST /auth/token`                                        | Access token may carry `active` next to the legacy `org` claim (§5), and `firstLogin.memberships` is built from the same centrally resolved client/product policy so it includes the selected cross-domain org/team. Code issuance, code consumption, and signing all revalidate the exact scope. Ordinary scoped refresh fails closed (and rolls rotation back) when its mapping or either membership is no longer valid; it never silently drops `active`, changes tenant, or auto-creates a product-domain workspace. The explicit workspace-switch grant is the sole scope-changing transition and requires an exact currently authorized org/team plus sufficient immutable family 2FA assurance. Same-domain sessions remain on the legacy policy even if a product key is revoked.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
5. **Silent workspace-switch grant — superseded/shipped** (§4.4) — security review produced the explicit exact-target custom grant with immutable family assurance and strict deterministic replay. The interactive `team_hint` flow remains the reauthorization path.
...
| switch grant | Exact-target custom grant, immutable refresh-family 2FA assurance, strict deterministic replay (§4.4)   | gap-fix A        | shipped 2026-08-13                                                                                                                                                                        |
| —            | Guests (§4.8), SCIM                                                                                     | product decision | not started (deferred, §12b)                                                                                                                                                              |
...
**Sidebar workspace stack.** `GET /org/me` is extended to return, per membership: `teamId`, `orgId`, `name`, `orgName`, `slug`, `iconUrl`, `avatarImageUrl`, `role`, `status`, `lastLoginAt` (from `LoginLog`, for ordering) — plus a top-level `pending_invites` array (same card data as the chooser). The sidebar is a straight render of this response: active workspace highlighted, others one click away, invite badges surfaced. Statuses other than `ACTIVE` are never returned here (a deactivated workspace simply disappears from the stack).

> **Shipped shape (gap-fix A, commit `a47aa45`; verified in `routes/org/me.ts` and `services/workspace-directory.service.ts`):** the response gains two new **top-level, additive** fields alongside the existing org-context payload — `workspaces: WorkspaceEntry[]` and `pending_invites: SidebarPendingInvite[]` — rather than the existing per-membership fields being extended in place. Each `WorkspaceEntry` carries `teamId`, `orgId`, `name`, `slug`, `orgName`, `iconUrl`, the non-null public `avatarImageUrl`, `role`, and `lastLoginAt`. There is **no explicit `status` key** on each entry: `buildSidebarWorkspaces` only ever queries `status: 'ACTIVE'` team memberships, so (as the paragraph above already promises) a non-`ACTIVE` workspace is omitted outright rather than returned with a status label. `avatarImageUrl` uses `<PUBLIC_BASE_URL>/teams/<teamId>/avatar`, resolving uploaded image → proxied `iconUrl` → generated fallback for both same-domain and product-authorized cross-domain rows. `lastLoginAt` is derived from `max(createdAt)` of the caller's own scoped `refresh_tokens` rows for that `(userId, domain, teamId)` — not from `LoginLog` as originally guessed — and is `null` for a workspace never opened with a team-scoped session; ordering is `lastLoginAt` DESC (nulls last) then `name` ASC. `pending_invites[]` entries are `{ inviteId, teamId, teamName, invitedBy, expiresAt }`, using the same `pendingInviteStatusWhere` eligibility filter as the chooser (`first-login.service.ts`). When an `all_active_memberships` caller has no same-domain org, the route resolves the access token's exact selected `active.orgId` through the live cross-product policy and uses that complete org context as the legacy wrapper; it never synthesizes an arbitrary org for an unscoped token. The directory can span organisations, so each workspace row's own `orgId`/`orgName` — not the wrapper's singular `org_id` — controls grouping.

**Switching.** Clicking another workspace normally submits the explicit backend-only custom grant described in §4.4 using that row's exact `orgId`/`teamId`, atomically persists the returned pair + selected workspace, and updates the product session. `403 INTERACTION_REQUIRED` falls back to `GET /auth?...&team_hint=<teamId|slug>` so the user can complete fresh proof; invalid/non-matching hints still render the chooser. `team_hint` also serves as UOA's equivalent of Slack's per-workspace URL: a product can deep-link `app.example.com/w/backend` straight into that workspace's sign-in.
...
3. `GET /org/me` enrichment: `orgName`, `iconUrl`, `avatarImageUrl`, `status` filtering, `lastLoginAt` ordering, `pending_invites[]`. **Shipped shape differs** — gap-fix A, commit `a47aa45`, plus the additive avatar URL contract: additive top-level `workspaces[]` (with `orgName`, `iconUrl`, non-null public `avatarImageUrl`, `lastLoginAt`, but no explicit `status` key — filtering happens by omission, ACTIVE-only) plus top-level `pending_invites[]`. See the full correction in §11.4.
...
7. `POST /auth/token` accepts the strict custom workspace-switch body and returns the ordinary pair scoped to that exact target. **Shipped** with immutable `RefreshToken.twoFaCompleted`, current target policy checks, strict scope-stable replay, and the three safe semantic error codes in §4.4.
...
8. **Explicit workspace-switch grant (2026-08-13).** The former deferral was resolved with a custom grant whose operation identity is the deterministic predecessor + exact target, not a caller nonce. Authorization-code TOTP proof becomes immutable refresh-family assurance. Valid target/policy/assurance conflicts roll back without consumption or revocation; ordinary refresh and replay can never return changed scope; post-grace predecessor use retains theft revocation.
