...
- Refresh rotation locks the exact organisation membership and then the exact team membership before creating its replacement. Lifecycle writers use the same order and commit their status tombstone plus scoped revocation atomically. A refresh-first race therefore leaves its replacement revoked by the waiting writer; a lifecycle-first race makes refresh fail without a replacement.
- Exact workspace scope is server-owned for recognized products and cannot be disabled by signed config flags. An originally unscoped auto flow or recognized product may bind one exact eligible ACTIVE workspace; multiple choices fail closed. Zero choices create a personal workspace only when `user_needs_team` explicitly permits placement, serialized by a per-user transaction advisory lock; otherwise a recognized product fails closed. Legacy same-domain/off exchange remains unscoped when its placement is already satisfied. Refresh rotation never runs placement.
...
7. **Cross-product refresh-family lifecycle linearization (2026-07-21).** A product-issued
   ...
   `uoa_admin`. Rotation takes the same ordered membership locks, closing replacement-after-revoke
   races, and reactivation/re-add never resurrects an old family.
