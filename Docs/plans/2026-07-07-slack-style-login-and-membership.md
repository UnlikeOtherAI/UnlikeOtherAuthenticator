...
- Refresh re-resolution validates the exact scoped membership and any product mapping. If the `active` org/team is no longer `ACTIVE`, or a cross-product mapping is revoked/expired/ambiguous, refresh fails closed without consuming or rotating the token. The user must restart interactive selection. It never drops `active` and silently switches or creates a tenant. Deactivation enforcement therefore needs no token-version bump.
- `user_needs_team` placement happens during authorization-code exchange, before refresh-token creation. An originally unscoped auto flow may bind one exact eligible ACTIVE workspace; a recognized product also reuses its sole cross-product workspace when selection is off, preventing a duplicate product-domain workspace. Multiple choices fail closed and return to interactive selection. With zero choices, the exact newly created personal org/team is validated, stored on the refresh token, and signed as `active`. Refresh rotation never runs placement.
