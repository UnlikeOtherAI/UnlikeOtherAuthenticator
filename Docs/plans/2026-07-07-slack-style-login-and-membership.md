...
3. **Social→chooser PKCE omission, found and fixed in gap-fix B (`6a226ab`).** `callback.ts`'s `workspace_chooser` redirect branch (added in the follow-ups round to route a social login into the chooser) initially omitted `code_challenge`/`code_challenge_method` from the redirect URL, even though `issueAuthorizationCode` mandates PKCE and the social flow's `socialState` already carried both values. This broke the social→chooser→`select-team` path at code-issuance time — the client's PKCE verifier could never be matched. The redirect now forwards both params (only when present) alongside the `login_token`. The adjacent 2FA (`twofa_token`) and 2FA-enrollment redirect blocks were audited at the same time and found correct by design: they embed the PKCE challenge *inside* the signed twofa/setup bridge tokens themselves, so they never needed to carry it in the URL — the chooser branch was the sole omission because its `login_token` carries only `userId`+`domain`. Route tests now assert the redirect carries PKCE when present and omits it when not.
